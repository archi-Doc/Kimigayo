...
$script:Tick = $null
function Invoke-Tick { if ($script:Tick) { & $script:Tick } }
...
function Copy-Value($Value) { ConvertFrom-Json -InputObject (ConvertTo-Canonical $Value) -AsHashtable -Depth 100 }
function Read-Json([string]$Path, [string]$Schema = '') {
    ...
    ConvertFrom-Json -InputObject $canonical -AsHashtable -Depth 100
}
...
function Save-Record([string]$HomePath, $Value, [string]$Directory = 'records') {
    ...
    if (Test-Path -LiteralPath $path) {
        if ([Autoframe.Json]::FileHash($path) -cne $hash) { throw "Corrupt record: $relative" }
...
function Read-Plan([string]$Root) {
    ...
    $plan=ConvertFrom-Json $json -AsHashtable -Depth 100
    ...
    @{ data=$plan; hash=[Autoframe.Json]::FileHash($path) }
}
function New-Logical($Plan) {
    ...
    @{schema_version=1;tasks=$tasks;milestones=@($Plan.milestones);findings=@();evidence=@();execution_plan=$null}
}
...
function Assert-Logical($Logical,$Plan) {
