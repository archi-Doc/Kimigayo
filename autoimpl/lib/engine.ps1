...
function Start-Frame([string]$Root,$Settings,$Explicit,[switch]$Resume,[switch]$NewRun,[switch]$ResetStallCounters,[string]$ResetReason) {
    ...
    try {
        ...
        while($script:State.status -ceq 'Running') {
            ...
            $script:State.attempts++; Save-State
            [Console]::WriteLine("$($script:State.attempts): $($Context.phase) $($Context.targets -join ',')")
            $childResult=$null
            try {
    ...
    [Console]::WriteLine("$($script:State.status): $($script:State.message)")
