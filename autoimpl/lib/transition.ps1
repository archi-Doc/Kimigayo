...
function Apply-Result($Logical,$Result,$Plan,$Context,[string]$Root,[string]$HomePath) {
    ...
    function Require-Proof($Ids,[string]$Target,[string[]]$Kinds,[switch]$Current) {
        ...
        foreach($id in $Ids) {
            if(-not $proof.ContainsKey($id) -or $Target -cnotin $proof[$id].target_ids -or $proof[$id].kind -cnotin $Kinds) { throw "Invalid evidence reference: $id" }
    ...
    foreach($p in $Result.progress) {
        if($Result.phase -cnotin @('Work','Verify') -or $p.task_id -cnotin $Context.targets) { throw 'Invalid progress target/phase.' }
    ...
    Assert-Logical $next $Plan -AllowIncomplete:($Result.phase -ceq 'Plan' -and $Result.decision -cin @('replan','needs_input'))
