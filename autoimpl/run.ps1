...
try {
    ...
    $settings=@{}
    foreach($key in @('MaxRunMinutes','MaxPhaseAttempts','PhaseTimeoutMinutes','SaveReserveMinutes','StopTimeoutSeconds','MaxTasksPerWork','CompletionAuditInterval','CodexCommand','PhaseModels')) { $settings[$key]=Get-Variable -Name $key -ValueOnly }
