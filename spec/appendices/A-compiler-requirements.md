...
After merging, keep each node's and fragment's original CodeContext and diagnostic and header locations, and resolve its bodies, headers, Types and Constraints in that context, not the merged Container's. Generated documents have their own contexts. Source-less roots and wrappers supply no alias environment and preserve the wrapped syntax's original context.

Lowering places a selected top-level runtime body in a private internal function that keeps its SourceDocument scope and CodeContext. It neither synthesizes a public `main` nor permits a top-level `return` (§22.2).

The internal name `MacroKoto` defines no language semantics; `$` is the Composition Root.
...
Mod execution and provisional Binding follow [§20.7](../20-compilation-configuration.md#207-mods-source-generation). After generation, final Binding keeps the following stages; Mod updates may repeat the affected analysis but never reopen a finalized program:
...
Keep separate Type/Value and Origin/Label tables. A Lookup Context keeps source, scope, namespace, role and access; its outcomes keep Symbols, function groups, deferred work or errors. Lowering never resolves strings again. Share normalized Types, but isolate candidate variables, constraints, mappings, tentative bindings, adaptation plans and rejection reasons, and commit only the winner. Early filters must preserve where lookup stops. Pairwise ranking may take O(n²) comparisons, excluding the cost of Type comparison.

Keep the normalized argument-name contract (external names and K, §7.2.2) separate from the source boundary position, Types, receiver position and defaults. Check that external names are unique across both sections and the receiver, independently of internal-name scopes. Preserve contracts and inherited specializations through source round trips and version-checked reloads; a changed contract invalidates the dependent call plans.

Cover absent, leading and intermediate boundaries, trailing commas, nested defaults and body dedents; reject repeated or empty boundaries, adjacent commas, removed suffix markers and boundaries in prohibited contexts. Test all name and default combinations, alias names, duplicate, unknown and missing arguments, positional prefix matching without skipping or Type-directed remapping, bound and unbound receivers at every position, constructors and base calls, requirements, overrides, specializations, function values and foreign headers. Check equivalent source positions with equal K, and prevent candidate merging when K differs. Verify source-order explicit evaluation, declaration-order defaults and cleanup through supported generation. Reject mixed language versions and incompatible old snapshots.

As implementation guidance, precompute positional limits with receiver handling, and reject excess positional arguments before inference without hiding declaration errors. Reuse name-to-slot lookup per declaration and generic family, and benchmark small linear scans against cached indices instead of allocating a dictionary per call. Name lookup never reorders evaluation. Measure allocation and timing on representative cold and warm cases without claiming universal zero allocation.

Pending conformance mappings are not proof evidence. Resolve the information lookup needs before computing effects; an empty or missing summary cannot fill unresolved work. Complete implementation proofs, public statuses, conformance and use obligations in dependency order. Effect closure cannot justify a cyclic conformance, and no unresolved ObjectCallCompatible status may be published. Keep ordinary errors separate from a valid implementation's NotProven status.
...
Do not reuse results across different roles or unrelated access Containers. Source, dependency, alias, selection or Symbol changes invalidate the affected caches. Loans and initialization affect Usage Legality, not overload-cache keys; refinement affects input Effective Types and therefore member and applicability keys.

Apply §21.3.4 to all declaration-dependent judgments, including absence dependencies. Test Name and specialization additions, access expansion, Signature changes and Proven withdrawal against stale artifacts. Reject consumers that lose a requirement, accept compatible changes after revalidation, and diagnose missing rebuild information without depending on load order.

Verify source isolation, merged private access, duplicates, roles and arities, Type/Value paths, aliases, unavailable extensions and modifiers, expected results, incomparable candidates, nested inference, generic environments and the absence of usage-failure fallback. For §2.5.1, cover declaration, requirement, constructor, deinit and accessor prefixes while keeping ordinary same-spelled Names and independent next-line declarations. Load, parse, generator-completion and candidate order must not change results.

After merging and inference, check recursive API access domains (§9.3), keeping dependent associated-Type obligations. Cover private Types in internal APIs, restricted enclosing Containers, private generic arguments and helpers across Kotonoha, sealed bases, open-fragment agreement, base access, protected receivers, compound access and the inherited-Name prohibition of §6.2.2. Include all ancestor layers, Property access independent of accessor access, conditional members, and fragment and Mod additions. Access caches depend on base graphs, modifiers and completed Container contents as well as aliases.

Keep parameter immutability, receiver index and kind, argument mappings and default environments (§7). Verify default Loans, partial-call cleanup and receiver-first evaluation independent of parameter position. Inherited lookup caches keep the committed layer; an applicability or accessor failure cannot add base overloads.

Persist the static effects and returned Loan anchors of §15.6.4 in artifacts and callable data. Use stable Field Identities, recursive summaries, and initializer and destructor dependencies; a summary change invalidates its callers. A missing or incompatible summary requires conservative effects or a diagnostic. Test Loans across direct, recursive, indirect and separate-module mutation, default and cleanup effects, and permitted shared reads. Runtime Origin erasure must preserve these proof dependencies.
...
`ptr` is not a Primitive Type. LLVM `ptr` is a backend representation, and instructions supply the Types needed for memory access and arithmetic. Lowering must preserve this specification and use properties such as `inbounds` only when their premises hold. Language undefined behavior and LLVM poison are distinct concepts. §21.5.3 specifies the initial instruction mapping; pointer/integer round trips add no provenance guarantee.
...
Analyze `defer` registration separately from execution: a non-completing cleanup affects exit paths, not the path after registration. Lower `defer` and destruction into one exit sequence that keeps only the needed registration state; no dynamic closure, function value or heap cleanup stack is required. The initial physical lowering and slot responsibility follow §21.4.

In an `if condition` whose body is only `defer => cleanup()`, the true branch registers and runs `cleanup` before its own exit, and the false branch does neither. This needs no registration flag, and lowering cannot move the cleanup outside the branch. (`cleanup` is an illustrative API.)

Keep the constructor choice, initializer environments, per-layer completion, component initialization and responsibility, and the exact layer of the destruction receiver. Cleanup destroys the base after the own fields, and a completed base keeps its destructor if derived construction fails. Interfaces keep constructor access and signatures, destructor presence, logical component order and exact dynamic cleanup chains. Physical offsets or apparently empty destructor bodies cannot replace these facts.

Verify duplicate `deinit` across fragments and selections; forbidden placement, modifiers and calls; implicit-constructor suppression; base-constructor access; completeness before and after constructor cleanup; partial Tuple and array construction; reverse element and base order; the absence of destruction accessors; and skipped Moved components. Also cover partial-value replacement, interrupted cleanup, receiver escape, rejection of base slicing and replacement, and exactly one cleanup of an object at its final reference.

## A.8. Callable and object verification

Verify OwnedOrigins through raw pointees, unused Type and Origin slots, captures and recursive payloads; test `a outlives static`, unresolved abstract Origins, Function Item bound arguments, and fixed callable signature Origins versus per-call binders. Reject mutable-static borrows at Owned boundaries, including after capture and across modules. Checked casts may supply as `static` only fixed Origin bindings that a proof covers; they preserve outer handle Origins and never bind per-call callable Origins. None of these checks may depend on a private body being available to a client.

Verify input-derived Callable results for all three receivers: per-call Origins, meets of several inputs, retained result Loans, rejection of a shared-to-exclusive upgrade, and rejection of hidden-receiver or call-local result dependencies. Common Function Type compatibility uses the same argument and result relation and keeps its separate Owned-environment restriction.

Keep capture bindings, acquisition order and effects, mutability, nested dependencies, environment identities, and capture, call and result Origins and Loans through compilation and artifacts. Resolve the required Copy, receiver, Callable and erasure obligations before finalization. Check concrete Copy independently of receiver kind and erased-container classification.

Verify the single public status per call operation of §12.4.4, the distinction between standard Places and witnesses, and the common implementation-family guarantee. Cover Whole and Base replacement, incomplete MoveOut, exclusive escape, permitted Part replacement and exchange, reference-field versus referent aliases, captures and statics, defaults and cleanup, returned borrows, and separate, indirect and generic calls. Verify recursive fixed points without treating a circular conformance as evidence. A NotProven specialization changes the public guarantee without causing a declaration error for that reason, and its cause must be traceable at a failing use. Unknown, missing required data and unfinished verification cannot supply proof.

Verify multi-level generic base projection, original protected-receiver checks, shared and exclusive access, returned-Loan anchors, direct Field access, and rejection of whole-base replacement, consuming receivers and unbound derived-to-base argument conversion. Preserve these results across separate compilation and optimization.

Verify the Copy and Move input states of object creation, payload eligibility, fresh identity, the initial strong count, allocation failure and source-storage responsibility. Strong-owner duplication must preserve identity without retaining a Loan on the source handle's storage; cover count overflow, release through different views, exactly one final cleanup and rejection of resurrection.
...
Keep `require`'s condition, failure body and form, and layout as a statement; resolve transfers before any Selection-style lowering. Distinguish Requirement Tests from runtime `is` by syntax context. Prove failure non-continuation independently of condition truth, including caught transfers and actual cleanup paths.

Keep the Binding Identity, Value Instance, Effective Type and validity conditions, and coordinate refinement with initialization, Loans, transfers, cleanup and reachability. Cache Effective Types. Facts cannot cross old aliases, Boolean variables, later defer-registration states or Function Boundaries. Short-circuit and loop joins must not depend on analysis order.

Cover same-line and next-line `else`, comments, multiline conditions, missing or empty bodies, forbidden expression placement, both `is` contexts, single evaluation, trivial tests, invalid targets, early transfers, contradictory facts, Move and Replace invalidation, surviving member mutation, alias versus new-binding typing, failure validation even for `require true`, the retained static true successor of `require false`, and unchanged acceptance under optimization. Type-checking continuations must preserve valid refinements without restoring Moved values or ended Loans and without adding execution edges; they include their result constraints without merging their state into live paths.

## A.10. Enum and Pattern verification

Preserve Case Symbols, Pattern positions, guard and body Binding Identities, guard and body binding Types, access modes, acquisition effects, and Origins and Loans through compilation; the [reference plan](B-reference-models.md#b6-match-binding-plan) shows possible storage. Resolve Cases, Copy obligations and dependent reads by their deadlines. A Type alone does not identify owned versus shared access.
...
Keep Contract and Requirement identities, parent edges, associated-Type declarations, projections and specifications, requirement signatures and Constraints, and conformance mappings separate from ordinary members. Collect Constraints before resolving projections, and normalize explicit associated-Type identities before matching; never infer them from implementations. Origin-free matching keys must keep the Origin contracts elsewhere.

Verify requirement access and parameters, parent cycles and conflicts, duplicates before matching, selection before compatibility, conformance access domains, and consistency of explicit and implied parent mappings. Cache projections and candidates with their conformance and Constraint environment. Shared-candidate proofs must cover every substitution independently of optimizer code sharing.

Test qualified and ambiguous associated Names, diamonds and independent declarations, inherited and missing Type specifications, input and Origin matching, getter covariance and setter structure, redundant and conflicting parent mappings, and static calls and unsupported runtime Views. Cover safe and unsafe functions, defaults, nonstrengthening Constraints and effects, and public conformance with private implementations.

Conditional conformance checks also cover positive-only syntax, permitted block declarations, Type-use versus conformance premises, condition-scoped signatures, duplicate direct registrations across fragments, cyclic and Unknown evidence, and parent-path agreement. Compare applicability only after committed lookup and substitution; test same-group alternatives, no base fallback, no condition-strength ranking and no instantiation-time reselection. Verify conditional Copy derivation and Field bridges under their declared premises. Artifacts and cache invalidation preserve conditions, mappings, associated-Type bindings and proof dependencies independently of environment directives.

Test struct-conformance inheritance under §8.4.4: a compatible Utf8Format-like mapping succeeds, while `Self` arguments or results and owning receivers fail that path without invalidating the derived declaration. Cover fixed associated-Type normalization, alternative paths, intrinsic exclusions and the Unknown/Error distinction. Across A -> B -> D, keep A's Member Identity and `Self` and the composed Type, Origin and receiver mapping. Diagnose explicit conformance and constraint-use failures with the failed requirement and its cause, without warning on the open base. Generic calls use the kept mapping, not caller-side member lookup.
...
Distinguish missing targets, ineligible Semantics, suffix order, Origin expressions, use positions and name-resolution errors. Guide invalid grouped suffixes to `ref/T? during a`, and a Type-only `from a` to `during a`. For a Type-only `during a and …`, explain the intersection parentheses. In a requirement conjunction whose right simple name or projection is invalid as a requirement but valid as an existing Origin, add the same hint without rebinding or reparsing. If both roles are valid, a lint may warn, and the explicitly grouped `T is (ref/U during a) and B` suppresses it. Newline hints apply only to erroneous annotation-like continuations, never to valid ordinary Names.

An owned value's bare name cannot denote an outer borrow Origin, even when the value carries internal dependencies. Suggest a declared `x.slot`, or a local Borrow with inference such as `let r = x@ref` when storage borrowing is intended. `during (x@ref)` is not an Origin expression, and no suggestion extends a lifetime. Result-default diagnostics follow §15.4.3.

For an erroneous `Name{…}/…`, explain the removed borrow spelling while keeping the selected parse. In adaptation, supplement a syntax or Type error only when the head's Semantics role is established; `x@s/T` may infer a borrow Origin, but `x@(s/T during a)` still cannot prescribe one. Valid set-plus-division expressions receive no legacy warning. Reuse the once-parsed brace range and its immediately following token instead of a new `{…}/` lookahead scan; only this additional shape check is O(1), and Origin parsing and lookup keep their normal costs. Attachment can keep the current AnnotatedType's first Semantics directly, without grouping traversal or a duplicate-state table. Allocate explicit source-annotation data only where needed, and never mutate shared resolved Types with occurrence-specific syntax.

Verify equality substitution before result completion, normalized nontrivial explicit outlives relations, and deterministic fallback for result equality classes. Test tautologies, both orientations of an equality, upper, lower and composite bounds, declaration-fixed result quantification, and rejection of nested callable input quantification. Composite right-hand meets must not become independent edges. Verify local initializer inference, equality-complete declarations without initializers, and no reopening from later assignments. Fields may not introduce hidden public preconditions, and Phantom Origins may not manufacture Loans.

Keep the canonical contract of §15.3.7, including independent anonymous slots for each input Type occurrence, conditional reconstructed-pair slots and fixed nested dependencies. Apply stored and specialized contract inheritance before omission. Verify anonymous versus explicit correspondence, rigid required binders, fixed `static` counterexamples, nested signature boundaries, Owned-based aggregate result defaults and the absence of body-derived input conditions. Origin conditions must survive rechecking, separate compilation and cache invalidation, and inactive slots must not erase inner dependencies.
...
Test specialization ambiguity before contract checking; inherited defaults and Safety; closed arguments; Origin-only duplicate keys; `obj/D` versus `rc/D`; mandatory selection through shared callers and references; recursion; and no fallback. Verify unused selected declarations, closed-set ownership, and set and body invalidation. Code sharing, budgets, automatic specialization and LTO must preserve behavior. Cover rejection of unconstrained reuse and capture, fixed Property results, element Places without Copy-dependent result Types, rejection of bare acquisition without Copy evidence, caller-independent definition checking and the absence of hidden applicability conditions. Instantiation preserves the symbolic ownership and cleanup plans of §8.10.

## A.13. Sequence access verification

Bind index versus range access by argument Type, not by literal syntax. Keep receiver and element Places, access form, bounds, permissions, acquisition, complete result Type, and Origins and Loans. Nested access cannot read or copy intermediate arrays. Element Places carry the stored Type; validate every admitted generic case before lowering.

Preserve one-time receiver and argument evaluation, index-evaluation protection, exclusive activation after bounds checks, and right-hand-side-first simple and compound assignment. Apply the common increment and exchange rules. Metadata snapshots add no source Loans and waive no receiver validity checks.
...
Validate windows-x64-v1 with LLVM 22.1.8, distinguishing semantic acceptance, TypeLayout, IR structure, object ABI, dependencies and unwind information, and actual execution. The LLVM verifier alone cannot establish layout, foreign ABI, ownership transfer or correct startup. Keep input, target, settings and expected results together; keep representative golden IR plus structural checks, and normalize only irrelevant internal numbering and paths.

IR and signature expectations for internal functions test the selected compiler implementation, not a stable language ABI. When its choices change, update those expectations and verify matching definitions, calls and adapters, artifact invalidation, source-order acquisition, ownership and result delivery, and unchanged external contracts. No cross-build internal ABI compatibility test is required.

The rows **Metadata and sharing** (its sharing parts), **Generic entry ABI**, **Generic context and fixed facts**, **Generic scratch frames** and **Generic budgets and reuse** apply to the deferred generic code sharing (§21.3.1). The initial monomorphizing profile verifies explicit selection, generic lengths and destruction, and generation limits on its concrete bodies.
...
Use fault-injecting test adapters where needed without expanding the production set of six operations and seven APIs. Compare the O0 and O2 stdout, stderr, exit status and observable effect and cleanup order; compiler Debug and Release builds must preserve acceptance and runtime checks. Run nontermination tests with bounded test-harness time and inspect the optimized CFG. Separate required constant evaluation from ordinary constant expressions, and diagnose unsupported generated operations before optimization, even in unused bodies.

Adopt a helper supply only after `/NODEFAULTLIB` custom-entry O0 and O2 links and executions establish that there is no hidden CRT-startup, C/C++ dynamic-initialization, TLS or exit-handler dependency. Check every actual undefined symbol of the objects, including references LLVM adds or removes, against real providers. Version, hash and catalog mismatches fail validation. Validate memory-intrinsic expansion, libcall paths and assembly unwind information.

The first executable must itself produce exactly `Hello, world!\n` (14 UTF-8 bytes), empty stderr and exit code 0; host test-runner output is not a substitute. Also test empty strings, Japanese and NUL bytes under redirection, and Abort exit code 1. Library verification checks the retained pre-optimization bodies, signatures and linkage and object generation, not external linking or execution. Assess unnecessary slots, transfers and flags with representative O2 code without deleting semantically required work.

## A.15. Control-flow verification

Preserve the Body form, evaluation context, transfer target, result sources and the distinct analyses of §14.9. Verification must cover:
...
Use reverse dependencies and bounded worklists for invalidation. Bound graph size, work, concurrent verification and total memory, and do not make module depth depend on host-stack recursion. Valid semantic caches may load bodies lazily and avoid source decompression from integrity-checked managed input, but only after checking that mandatory verification completed, including for unused definitions. Reuse native copies and indexes by content. Compare alternatives with identical input and independent environment-specific state.

After verification, common generation may deduplicate equal plan keys, call statically selected implementations directly, propagate constants and omit unnecessary code and metadata. Do not demand full specialization for every Type or unconditional forced inlining. Introduce fine-grained reuse, laziness and parallel checks incrementally, preserving the no-cache semantics.
...
Measure retrieval, lexing, semantic verification, generation and linking separately, recording bytes read, hash passes, revalidated judgments, bodies loaded, allocations, peak memory, code size and runtime. Include many-path graphs, a large Library with sparse use, generation-only setting changes, and repeated small external and generic calls. These rules imply no measured speedup or numeric resource guarantee.
...
The [test execution profile](../testing-profile.md) defines the concrete formats and defaults. Verify solution-wide barriers and budgets, IDs that are stable between single-project and solution runs, an absent product entry, startup bodies that are checked but not executed, the temporary-directory API and environment, and bounded result retention. Membership and discovery, then serial verification and reporting, then cleanup and recovery, then parallelism and external output is a possible implementation sequence, not a reduced conformance requirement.

## A.18. Kimi library and source aliases

Verify the rules of §18.1, §9.4.1, §22.1 and §22.4 across parsing, Binding, analysis and generation:
...
Index named aliases per document, and reuse Container member indexes for opening aliases. Share fixed references and normalized bindings without conflating them with completed validation. Resolve paths once per valid Binding generation, and share default environments by defining module. Preserve use-site checks, and invalidate results when assumptions or dependencies change. Library implementation names should identify Kimi (for example, KimiLibrary); IntrinsicKind remains a property classifier, and processing names such as BuildCore are unrelated.
...
For §15.6.7, test implicit and direct explicit reservations, transparent parentheses, object projections, generic, indirect and Callable calls, constructors and prepared defaults. Check overlapping exclusive reservations, shared arguments retained at activation, ancestor authority, disjoint static paths, nested dependencies and cleanup effects. Storage, captures and control-flow results must not propagate reservations. Test abandoned calls, locally caught transfers, Never and Abort, and unreachable checking. Corrupted reservation or activation plans must not reach emission, and O0 and O2 must preserve evaluation order, cleanup and the call-wide attribute contract.

Payload tests must keep Identity, allocation, Dynamic Type, header, counts and Descriptor, and the final release destroys the current contents once. Compare O0 with O2 and shared with specialized generation, including invalidation of refinements that depend on old contents. Lowering and Emit consume checked address, acquisition and transfer plans, reject stale or incomplete plans, and never rediscover completeness from optimized code.
...
Verify zero documentation-specific allocations with collection disabled, and no dedicated candidate buffer when none is needed. Include an ordinary-compilation baseline; parser-only improvements do not establish a compiler-wide speedup. Keep external callbacks, declaration and source-name inputs, generated output and optional fine-grained mappings visible in resource accounting. No unconditional speedup or fixed parser architecture is required, but observed costs must meet the profile's bounds and the recorded acceptance gates.

## A.22. Optional Types, propagation and discard

Implement §3.2.3, §14.2.4, §14.6, §17.2.4 and §17.4 consistently across lexing, parsing, Binding, control flow, ownership, lowering and generation.

- Preserve optional, `try` and discard syntax and source locations. Normalize optional syntax to the recognized Option Identity before proofs, decomposition and layout.
- Check prefix and arrow binding, `?` before postfix `during`, repeated `?`, grouping boundaries, generic arguments and adaptation targets after Optional expansion. Include `@(ref/T during a)?`, reject outer-chain Origins and suffixes on Semantics shorthand, and verify ordinary contextual-keyword names and error recovery.
- Recognize a standalone `_` and `try` only after scanning the whole identifier; keep all specified underscore uses and reject ordinary names.
- Resolve `try` operands without expected Types. Check both Cases, one-layer extraction, distinct success and error Types, incompatible return targets, unreachability, nested boundaries, anonymous results, generics and Never.
- Route propagation through ordinary match acquisition and return cleanup before generation. Verify each evaluation and destruction count, retained borrowed payload dependencies, partial argument and aggregate construction, deferred cleanup, and Abort and non-completion.
...
- Diagnostics explain mismatched success and failure paths and valid alternatives, without speculative fix-its or unlimited overload retries.

Reuse the existing Type, Case, match, return and cleanup machinery without textual duplication or hidden Closures. As-if copy and slot elision must preserve acquisition and Move state, dependencies and destruction order. Cleanup sharing requires matching state, defers, secured results and destinations (§21.4.5), not merely equal scope depth. Do not assume failure paths are cold without evidence, and add no niche optimization beyond the nonnull Option representation of §21.1.5. Old caches and artifacts must not certify changed rules.
