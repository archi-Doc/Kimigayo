...
| Subject written bare, `x@ref`, as a stored/parameter `uniq/T`, or as `x@uniq`/`x@objuniq`/`x@uniq/T` | Shared borrow or shared Reborrow that leaves the Place and borrow value usable; reject each exclusive spelling at the Subject without further diagnostics |
...
| Metadata and iteration | Receiver effects, completeness checks, no result Loan for metadata, stable saved indices, reference iteration independent of element Copy, no iterator-owned borrowed results; `ref`/`uniq` Array, fixed-array and Slice iterables shared-reborrowed with their Loans kept through the loop; `@uniq` iterables rejected |
