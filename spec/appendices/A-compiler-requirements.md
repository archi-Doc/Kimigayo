...
| Subject written bare, `x@ref`, `x@uniq`/`x@objuniq`/`x@uniq/T`, `x@move`, `x@owner`, a temporary, or a reference value | Ordinary acquisition except that a bare Place is borrowed; Shared, Exclusive or ByValue mode by the access of the Subject Place: a temporary is ByValue, a `uniq` value Exclusive, `@uniq` on a `let` slot is rejected, a Copy Subject under `@move` is transferred and `x@owner` copies it |
