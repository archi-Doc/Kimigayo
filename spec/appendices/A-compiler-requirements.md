...
Retain parameter name-omission permission independently of default-expression presence (§7.2), including in published declaration metadata and reconstructed call plans. Verify all four declaration forms against named, positional and omitted arguments; external-name marker placement; no positional skipping or Type-directed remapping; duplicate labels; and defaults before required parameters. Cover bound/unbound receivers, constructors/base calls, requirement-selected and override-selected name permissions, inherited specialization contracts, positional-only function values, and foreign imports that permit `?` but reject defaults. Defaults with and without `?` use the same declaration checking, generic-inference exclusion, evaluation order, ownership and cleanup rules. A name-optional parameter without a default must never create a default operation. Changed labels, name permissions or defaults invalidate dependent call plans even though they cannot independently distinguish overloads.
