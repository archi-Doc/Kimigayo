...
| Associated Type | A complete-Type binding fixed by explicit Type-identity facts, optionally with Origin parameters applied positionally as `LentItem(a)`. | [Associated Types](../08-generics-constraints-and-contracts.md#843-associated-types) |
...
| Subject mode | Shared, Exclusive or ByValue access of a `match`/`for` Subject, given by the shared, exclusive or owned Subject Place that its acquisition produces. | [Subject rule](../15-ownership-and-lifetime-analysis.md#1516-match-acquisition-and-lifetime) |
| Item / `LentItem(step)` | The value an Iterator delivers per `next`, whose Type is independent of the call / the item of a LendingIterator, whose Type may depend on that call's receiver borrow `step`. | [Iterator](../22-core-execution-and-foreign-functions.md#2212-iteration-cursors-and-storage) |
...
| Iterator / LendingIterator | The refinement whose step-independent items survive later `next` calls and Moves under a published effect bound / the Contract every enumeration uses, whose items may borrow the Iterator. | [Iterator independence](../22-core-execution-and-foreign-functions.md#22124-iterator-independence) |
...
| Common adaptation | The single operation selected at a position with a fixed expected Type: borrow, Reborrow, one shared reference through nested layers, Scalar read or acquisition. | [Common adaptation](../10-overload-resolution-and-inference.md#102-common-adaptation-at-expected-types) |
...
| Receiver Expression | The expression in the receiver position of a method, accessor or direct Closure call; acquired implicitly by the receiver requirement. | [Value model](../03-types-and-values.md#34-values-places-and-storage), [receivers](../07-functions-and-callable-values.md#73-explicit-receivers) |
