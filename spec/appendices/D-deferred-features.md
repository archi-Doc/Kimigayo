...
| Mods (source generation): host interface and configuration | Deferred. Query, marker-registration and context APIs, packaging, project configuration and cache formats are undefined, and until they are specified no Mod is registered or executed. The execution and semantic rules for that host remain specified | [Mods](../20-compilation-configuration.md#207-mods-source-generation) |
...
| Local automatic-specialization hints | Deferred until the global multiplier cannot meet measured local needs; a future hint must be safe to ignore and preserve semantics and resource limits | [Generation limits](../21-layout-runtime-and-code-generation.md#2135-generation-limits-and-code-merging) |
...
| Guard candidate capture | Define explicit syntax and timing for acquiring a read value, and Origin/Loan escape checks; no `@copy` form exists. `candidate@deref` copies a Copy value under the ordinary rules (§14.8.3). |
