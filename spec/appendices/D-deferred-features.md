...
| Private-source binary distribution and stable external Kimigayo ABI | Deferred; source packages, semantic records, and local publication are specified | [Source artifacts and binary interfaces](../18-modules-and-dependencies.md#183-source-artifacts-and-binary-interfaces) |
...
| Notification/USN-based skipping of mutable-source reads | Deferred pending proof of complete observation and snapshot synchronization | [Input reuse boundary](#d3-input-monitoring-and-reuse) |
...
Mutable input is fixed from actual bytes (§18.5.2); file ID, size, and timestamp equality are not content evidence. Notification cookies and per-file USNs do not yet replace reading it. Before adopting such a path, validate initial full scanning, notification completeness/order, rename and alias updates, open writing handles, and races between synchronization and snapshot capture. Overflow, watcher disconnection, cookie failure, or journal-generation changes must cause a full reread. Cookie delivery or a last-recorded USN alone is not a portable proof of no changes. This deferred optimization does not weaken the distinct compiler-only-write assumption for registered user-cache content (§18.6.4).
