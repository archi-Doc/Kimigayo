...
| Guard candidate capture | Define explicit read-value acquisition syntax and timing and Origin/Loan escape checks; no `@copy` form exists. `candidate@deref` copies a value under the ordinary rules (§14.8.3). |
| Pattern-local acquisition selectors | A Pattern binds a transferred value, `ref/T` or `uniq/T` by its path (§15.1.6); a per-binding selector such as a Pattern-local `@move` or `@ref` is not introduced. |
