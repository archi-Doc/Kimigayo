...
**Complete Sealed payload exception.** A selected `ref/Self` receiver may use the complete payload projection of §13.5.5.1 implicitly when the source View Target is exactly the same complete Sealed Type; a `uniq/Self` receiver uses it only through the explicit projection, as in `(handle@uniq/T).method()`, because an owned handle is never lent exclusively without a spelling. Either is an ordinary complete-value call and needs no ObjectCallCompatible proof; any existing NotProven public status stays unchanged. Ordinary argument positions always require an explicit projection.
