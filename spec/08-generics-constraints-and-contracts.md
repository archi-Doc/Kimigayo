...
Omitted specialization Origins are inherited, not quantified. First match normalized input structure and map pending Origin positions to each candidate; select exactly one target without Origin conditions or results breaking ties. Then complete from that contract and validate explicit bindings, result, formation, bounds and Loans. A failed validation never retries another candidate. After inherited binders are matched, the body must work for **every Origin binding permitted by the original contract**. A specialization cannot add an Origin parameter or lifetime restriction, narrow applicability by Origin, or register another implementation for different Origins. It cannot rescue an invalid ordinary implementation or replace a declaration-required proof; [deferred generic checking](#810-generic-body-checking-and-deferred-obligations) keeps its stated design boundary.
