...
$source = Join-Path $repo 'milestones/Milestone13.kimi'
$sourceHash = (Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash
$compilerHash = (Get-FileHash -LiteralPath $compiler -Algorithm SHA256).Hash
$work = Join-Path $repo "bin/milestone13/$Configuration/$([guid]::NewGuid().ToString('N'))"
...
$invalid = [ordered]@{
    ElementI32 = @{ source = (Edit-KimiSource $original 'associate Iterator.Item is ref/T during source' 'associate Iterator.Item is i32'); diagnostic = 'IncompatibleContractImplementation_Kd' }
...
Write-Output "Passed $($results.Count) Milestone 13 checks ($Configuration): $report"
