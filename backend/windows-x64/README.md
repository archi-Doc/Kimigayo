...
# Later backend regeneration uses the installed tools:
./backend/windows-x64/build.ps1
```

Projects require neither LlvmBin nor a kimi_backend entry. Source-built compilers find this checkout's toolchain from their executable location; standalone distributions use toolchain beside Kimi.exe/Kimi.dll. `--ToolchainRoot` (scripts: `-ToolchainRoot`) or `KIMI_TOOLCHAIN_ROOT` selects another root. Legacy `--LlvmBin`/`-LlvmBin` only overrides LLVM tools. No PATH lookup occurs. See [SPEC §20.8.8](../../SPEC.md#2088-toolchain-storage-and-native-library-lifecycle) for storage, resolution, generation, installation and Application build details.
...
Candidate outputs remain under ignored `bin/`: the native `.lib`, per-member COFF/unwind/disassembly reports, C-generated LLVM test IR, O0/O2 objects/executables, and `verification.json`. Only a pinned, tested reproduction matching the catalog is installed at `../../toolchain/windows_x64/kimi_backend_windows_x64_v1.lib` (or the selected root). Exploratory/different-hash candidates warn and leave the installed library untouched. A failed run leaves `status: incomplete`; old files do not establish current success. Executable tests have a 30-second timeout.
...
Rebuilding is repeatable from these sources and recorded tools; adoption must compare the resulting archive hash rather than assume filename or timestamp equality. The build script installs only a verified reproduction into the local toolchain; it does not publish a runtime package or adopt a new archive.
