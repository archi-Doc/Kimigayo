...
Kernel32 imports are now generated from the project-owned kernel32.def with the pinned LLVM 22.1.8 llvm-dlltool binary. Its SHA-256 is recorded in profile.json and matches the verified distribution and the selected C:/App/llvm copy. The generated library supplies ten named x64 imports; no SDK library is selected. The four helper archive members and adopted archive hash are unchanged. Local reports in `backend/windows-x64/bin/verification.json` retain tool/source/library hashes; generated-module reports are in `bin/emission-native/{Debug,Release}/verification.json`. These reports are generated evidence, not committed binaries or fabricated supply identities.
