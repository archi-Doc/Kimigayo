...
$source = Join-Path $repo 'milestones/Milestone32.kimi'
$sourceHash = (Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash
$compilerHash = (Get-FileHash -LiteralPath $compiler -Algorithm SHA256).Hash
$work = Join-Path $repo "bin/milestone32/$Configuration/$([guid]::NewGuid().ToString('N'))"
...
if ($Cases -eq 'All') { Build-And-Run $source (Split-Path $source) 'Milestone32' 'O2' $expected }
$original = [IO.File]::ReadAllText($source).Replace("`r`n", "`n")
$variants = [ordered]@{
    Unchanged = @{ source = $original; stdout = $expected }
    Numbers = @{ source = (Edit-KimiSource $original 'Reading.init(42)' 'Reading.init(-9223372036854775808)' 'reading.number == 42' 'reading.number == -9223372036854775808'); stdout = $expected.Replace('Reading(42)', 'Reading(-9223372036854775808)') }
    NulBytes = @{ source = (Edit-KimiSource $original '"Reading(\(self.number))"' '"Reading\0(\(self.number))"'); stdout = $expected.Replace('Reading(', "Reading$([char]0)(") }
    Empty = @{ source = (Edit-KimiSource $original '"Reading(\(self.number))"' '""'); stdout = $expected.Replace('Reading(42)', '') }
    Failure = @{ source = (Edit-KimiSource $original 'return $tryWrite(writer, "Reading(\(self.number))")' 'return .Err(BufferFull.init())'); stdout = ''; stderr = "Failure.kimi:13:12: abort KIMI_E_FORMAT: Formatting failed`n"; exit = 1 }
}
foreach ($level in @('O0', 'O2')) {
    foreach ($entry in $variants.GetEnumerator()) {
        ...
        Build-And-Run $project $directory $name $level $variant.stdout $variant.stderr $variant.exit
    }
}
$invalid = [ordered]@{
    MissingConformance = @{ source = (Edit-KimiSource $original "    Self is Utf8Format`n" ''); diagnostic = 'NoApplicableOverload_Kd' }
    MissingConstraint = @{ source = (Edit-KimiSource $original "    T is Utf8Format`n" ''); diagnostic = 'UnprovenConstraint_Kd' }
    BareWriter = @{ source = (Edit-KimiSource $original '$tryWrite(writer@uniq, "君: \(text)")' '$tryWrite(writer, "君: \(text)")'); diagnostic = 'ExclusiveBorrowRequired_Kd' }
    RootSourceAccess = @{ source = (Edit-KimiSource $original '"君: \(text)"' '"君: \(bytes[0])"'); diagnostic = 'ComparisonLoanConflict_Kd' }
}
foreach ($level in @('O0', 'O2')) {
...
Write-Output "Passed $($results.Count) Milestone 32 checks ($Configuration): $report"
