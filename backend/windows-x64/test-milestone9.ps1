...
$variants = [ordered]@{
    ...
    Absent = @{ source = $original.Replace('let target: i32 = 6', 'let target: i32 = 99').Replace('.Missing => $abort("Expected a match")', '.Missing => ::Core.writeLine("No match.")'); stdout = $expected.Replace('Found 6 at index 3.', 'No match.') }
