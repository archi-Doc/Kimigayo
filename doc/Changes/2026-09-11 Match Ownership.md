...
During investigation, the existing WarmBindingAndBothAnalysesReuseStorage test intermittently reported 7,856 bytes in Debug. Temporary phase instrumentation located the allocation within Ownership.Analyze, rather than Binding, but did not establish its cause. Targeted runs, subsequent full runs and a diagnostic run with tiered compilation disabled passed. No compiler instrumentation or runtime configuration override is retained. The test now records warm/measured Binding and ownership byte deltas for a future failure; the measured operations, four iterations and zero-byte requirement are unchanged. This observation is not evidence of a fixed allocation cause or a throughput result.
