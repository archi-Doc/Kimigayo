...
内部関数ABIは同一生成モジュール内のコンパイラー管理契約であり、固定しない。Libraryの保存用IRも同じ方針に従い、公開モジュール間ABIにはしない。calling convention、引数・結果の物理表現、順序、隠しcontext、ABI属性はコンパイラーが関数ごとに選択できる。変更に言語versionの改訂や公開内部ABI番号は不要で、異なるbuildの生成物の互換性は保証しない。定義・call・adapter・生成runtime helperを同じFunctionAbi契約に一致させ、非互換cacheを無効化する。cccも選択肢の一つであり、その採用だけでaggregateがC互換になるわけではない。

### 10.2. 物理的な引数・戻り値

次の表と§10.3の具体的な署名は、当初の方式を説明する非規範的な実装例である。型のstorage配置から関数の受け渡し方式は決まらず、直接・間接渡し、分割、結果slot、省略などを実装が選択する。未実装のValueLoweringや所有権処理を推測して出力してよいという意味ではない。
...
引数の評価・取得はsource順に一度ずつ行い、receiverは言語規則に従う。物理引数順は論理引数への対応を保持したうえで変更でき、診断位置などの非公開contextの位置も実装が決める。

この実装例では戻り値領域も引数領域も通常のptrとして明示する。byvalやsretを含む別の方式も、backend上の意味と必要な前提を満たす場合に選択できる。FunctionAbiと全利用箇所を一貫して更新し、言語上のCopy・Move・alias許可をABI属性から追加しない。
...
以下の責任移転は物理ABIによらず維持する。slotは論理的な取得値・確保済み結果の説明であり、各値に専用の物理領域を必須とするものではない。
...
未対応診断は§4.6の最適化前の生成対象に適用し、定数伝播で消える式や実行時の未選択経路も同じ扱いにする。言語が要求するコンパイル時評価・literal fittingは別であり、そこで確定した定数の格納・使用はこの制限の対象外とする。
...
f32/f64はIEEE 754、丸めは最も近い値・中間は偶数側とする。NaN、無限大、符号付きゼロ、非正規化数を扱い、通常の式を勝手に再結合・融合しない。
...
```text
生成結果の依存
├─ コンパイラー / レイアウト方式 / 内部ABI実装と関数契約の識別情報
...
通常関数間のhandleの受け渡しは、§10のコンパイラー管理ABIで選択する。専用slotへのポインターは実装例であり、stringのstorage配置から必須にはならない。物理的なビットの転送だけを、言語上のCopyとして数えない。Move後は元の束縛から所有権・破棄責任が移る。
...
- 実装が選択したscalar・aggregateの受け渡し方式、calling convention、ABI属性が定義・call・adapterで一致する。物理方式の変更時は実装用のIR期待値とcache識別情報を更新し、取得順序・所有権・結果の引き渡しと外部ABIを検証する。過去buildの内部ABIとの互換性は要求しない。
