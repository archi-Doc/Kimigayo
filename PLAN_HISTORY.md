...
- [Raw pointer subplace continuation (2026-09-22)](#raw-pointer-subplace-continuation-20260922)
- [Raw pointer continuation (2026-09-22)](#raw-pointer-continuation-20260922)
...
<a id="origin-redesign-native-20260921"></a>
## Origin redesign native verification (2026-09-21)

After the user installed the toolchain, the native checks blocked in the preceding
record passed against source revision `b10af47`. No compiler, specification, draft
or test-source correction was needed. Only verification/status documents changed.

The scalar runner checked LLVM 22.1.8 identities, the pinned backend archive hash,
input and optimized IR verification, allowed object dependencies, linking and exact
stdout/stderr/exit results. The three `OriginRedesign*` fixtures and the two
`Anonymous*Origin*` fixtures generated by the preceding Release managed run passed
all **10 O0/O2 executions**. The latter cover anonymous aggregate Origins and
anonymous-Origin specialization; they do not establish general specialization support.

```powershell
backend/windows-x64/test-scalars.ps1 -FixturePattern 'OriginRedesign*.ll' -OutputDirectory bin/origin-redesign-native
backend/windows-x64/test-scalars.ps1 -FixturePattern 'Anonymous*Origin*.ll' -OutputDirectory bin/origin-anonymous-native
backend/windows-x64/test-milestone14.ps1 -Configuration Release
backend/windows-x64/test-milestone16.ps1 -Configuration Release
backend/windows-x64/test-milestone14.ps1 -Configuration Debug
backend/windows-x64/test-milestone16.ps1 -Configuration Debug
```

`DOTNET_CLI_HOME` was set to `kimigayo-origin-dotnet` under TEMP. The initial sandboxed
scalar attempt could not execute `opt.exe` (permission denied). Approved execution
outside the sandbox succeeded; this was an execution restriction, not a compiler
or toolchain defect. NativeAOT was not run, and the unchanged managed suites were
not repeated in this follow-up.

| Suite | Debug | Release | Coverage per configuration |
| --- | ---: | ---: | --- |
| Milestone 14 | 48 passed | 48 passed | 39 execution checks and 9 required rejections |
| Milestone 16 | 57 passed | 57 passed | 45 execution checks and 12 required rejections |

The milestone scripts check direct native execution and both CLI run paths,
O0/O2 variants, link records and rejection before emission. Milestone 14 covers
the object/Callable pipeline and cleanup; Milestone 16 covers retained external
Origins, intersection results, equality, competing writes and escaping locals.
All four reports have `status: passed` and retain compiler/source SHA-256 identities:

- `bin/milestone14/Release/e6a5ed28c8524ea4b48b9077102cf445/verification.json`
- `bin/milestone16/Release/49ed239cac83403bbd1ded7d6c725ec3/verification.json`
- `bin/milestone14/Debug/208fc6bae9404c81be57dcb37ebce6e1/verification.json`
- `bin/milestone16/Debug/bd4a830d959a451ea33e291c9c9bbace/verification.json`

Scalar logs are `bin/origin-redesign-native.log` and `bin/origin-anonymous-native.log`;
milestone logs are `bin/origin-milestone14-{debug,release}.log` and
...
<a id="raw-pointer-continuation-20260922"></a>
## Raw pointer continuation — 2026-09-22 05:07:22 UTC

Start time recorded before inspection; initial HEAD `77bd5038`, clean working tree. Read the current execution prompt, plan, repository instructions and owning specification. This request authorizes unfinished M/I work for at least 60 minutes; program checkpoints neither authorize implementation targets nor constrain stopping. No draft edits or NativeAOT tests.

### T26s — compound assignment through Copy scalar/pointer pointees (DONE)

§5.2–3 and §13.7.2: retain the pointer and old value before evaluating the RHS, reuse checked arithmetic, and write only after normal completion. Existing plain assignment remains RHS-first. No new value representation or safe Loan is introduced.

Reproducer: the new `PointerCompoundAssignmentEvaluatesTheAddressAndOldValueOnce` failed before the fix with 13 unsupported ownership operations; Binding was complete. Its helper initially omitted the independently required unsafe block (§7.5); correcting that test did not relax production checking. Positive execution covers observable target/RHS order, an RHS that overwrites the target, all ten integer compound operators, pointer-pointee displacement and f64 updates. Separate fixtures cover overflow, division by zero, invalid shift counts, and an RHS return with no write.

PASS: Debug/Release solution builds, zero warnings/errors, using `dotnet build Kimigayo.slnx --no-restore -c <Config> --disable-build-servers -m:1 -p:EmitCompilerGeneratedFiles=false -v quiet`. `dotnet xUnitTest/bin/<Config>/net10.0/xUnitTest.dll -parallelMode none -failSkips -result-xml bin/verification-20260922-050722/t26s-full-<config>.xml` passes 11,112 tests/configuration; focused `-class XunitTest.ForeignEmissionTest` passes 38. The direct xUnit runner replaces a test-platform invocation that ran zero tests and provided no evidence. Restore was unavailable because the sandbox denied reading user NuGet.Config; existing restored dependencies built successfully.

PASS: LLVM 22.1.8 verification, compilation, linking and exact output/exit/stderr for `test-scalars.ps1 -FixturePattern 'ForeignPointerCompound*.ll' -OutputDirectory bin/verification-20260922-050722/t26s-native`: five fixtures, ten O0/O2 executions. Sandbox denied starting opt.exe; the authorized escalated execution succeeded. `git diff --check` passes. General bool/aggregate/Non-Copy pointee and indexing obligations remain separate.

### T26t — bool pointer storage (DONE)

§5.2 and §21.1.4: pointer loads now share scalar/element i8-to-i1 truncation; stores share i1-to-i8 zero extension. Removed duplicate pointer writer cases. Only valid stored bytes 0/1 are covered; the implementation does not sanitize invalid unsafe memory. `ForeignPointerBool` reads zero-filled storage, writes both bool values, inspects their bytes and checks stride one. Its pre-fix run failed with five unsupported operations. `WarmPointerReadWriteAnalysisAndEmissionAllocateNothing` measures 0 allocated bytes over 128 warm ownership-analysis/IR-writing iterations containing bool reads/writes and integer pointer compound updates, after 100 warmups; this is not a cold-compilation or throughput claim.

PASS: zero-warning Debug/Release solution builds; `-class XunitTest.ForeignEmissionTest -class XunitTest.ScalarEmissionTest` passes 71 tests per configuration (`t26t-debug.xml`, `t26t-release.xml` in the same evidence root). `test-scalars.ps1 -FixturePattern 'Foreign*.ll' -OutputDirectory bin/verification-20260922-050722/t26t-native` passes 22 ordinary O0/O2 executions with pinned LLVM 22.1.8. The preceding T26s full-suite results predate this writer change; final broad regression remains required.

### T26u — pointer indexing and bound unsafe checks (DONE)

§5.3, §13.7 and A.5: bind signed isize indexing, reject other offset Types/Range/from-end/Unit-pointee indexing, and route it through the same pointer load/store/update path as dereference. Retain evaluated SSA address values before evaluating indices or update RHS expressions; never reload a subsequently modified pointer variable. The shared pointer helper introduces no synthetic Koto tree or safe Loan. Ordinary assignment remains RHS-first; compound assignment remains destination/read-first.

The new unsafe regression reproduced four incorrect acceptances: bound p+n, p-n, p+=n and p-=n bypassed InferLocalType, which had held the arithmetic permission check. BindingControlFlowTypes now explicitly marks them and pointer indexing as unsafe operations. `ForeignPointerIndex` verifies negative offsets, stride, printed evaluation counts/order, pointer reassignment/reread inside index and RHS scopes, and returns that suppress destination or RHS evaluation. Source probes were corrected to use named exits for do and an outer closing delimiter on its own line (§14 and §2.2); production parsing was not relaxed. One native run during diagnosis used the earlier successfully generated fixture after a later source probe failed; that run is not evidence for the failed probe. Final verification below uses the regenerated fixture (SHA-256 01926F6926DDE791B9BF0901C0A21DCC8FA2260AB63CE12A5DE3E178F13F291B).

PASS: zero-warning Debug/Release builds, all 11,128 managed tests per configuration (`t26u-full-debug/release.xml` and logs), 54 focused ForeignEmissionTest cases. Final `test-scalars.ps1 -FixturePattern 'Foreign*.ll' -OutputDirectory bin/verification-20260922-050722/t26u-native` passes 24 LLVM 22.1.8 O0/O2 executions. The full suites include the shared writer changes from T26t. Pointer increment itself remains forbidden; integer pointee increment is the next independent operation family.

### T26v — prefix/postfix integer pointee updates (DONE)

§13.2 and §13.7.2: reuse retained addresses, checked integer computations and existing prefix/postfix result selection for dereference and indexed targets. Pointer-value increment and bool/float pointee increment remain rejected by Binding. The new positive fixture failed before the implementation. It prints one target/index evaluation for each of four update forms and checks both delivered results and stored value; two additional fixtures check signed i8 underflow and unsigned u64 overflow.

PASS: zero-warning Debug/Release builds; 91 focused ForeignEmissionTest/ScalarEmissionTest cases per configuration (`t26v-debug/release.xml`); three fixtures, six LLVM 22.1.8 O0/O2 executions in `t26v-native`. Full T26u results predate this small update; final broad verification remains required.

### T26w — whole Copy aggregate and zero-sized pointee transfers (DONE)

§5.2–3, §21.1.1/3/4 and A.5: permit whole tuple/fixed-array/struct pointees only with proven Copy and Owned; keep Non-Copy, dependent/reference-containing and direct aggregate subplace access unsupported. Reuse AggregateLayout and the existing complete-byte transfer, including padding without reading it as a value. Unit and zero-sized Copy values preserve pointer/RHS evaluation and acquisition while accessing no bytes. Scalar and aggregate pointer lowering share explicit Type/address checks; aggregate stores retain their acquired source Place instead of pretending an aggregate has a scalar SSA value. Generation checks source initialization, Copy acquisition, matching layout and address/value dominance. Corrupted source/address plans produce no partial output.

The aggregate reproducer failed before implementation. The first implementation correctly failed existing value-flow validation because a tuple construction does not have a scalar value ID; the retained source Place fixes the plan rather than bypassing validation. `ForeignPointerAggregate` verifies mixed-alignment Copy struct fields including bool, independent snapshots across replacement, returned aggregates, nested tuples, fixed arrays and aggregate-stride indexing. `ForeignPointerZeroSize` verifies evaluation order for Unit plus empty Copy structs and zero-length integer arrays. Three Non-Copy rejection cases and two corrupted-plan cases pass. The aggregate warm-analysis/writing probe also measures zero bytes over 128 iterations after 100 warmups; no cold or throughput improvement is claimed.

Final boundary review investigated whether C layout could enter the new aggregate path. The added source completes Binding but remains unverified by the existing control-flow/ownership gate, so emission already rejects it. The test's initial expectation that ownership was complete was incorrect and was removed; its required assertion remains that no IR is published. No C layout support or new production layout restriction is claimed.

PASS: Debug/Release solution builds, zero warnings/errors. Full suites pass 11,142 tests/configuration in `bin/verification-20260922-050722/final-debug.xml` and `final-release.xml` (logs beside them), covering the final production code. One C-layout rejection test was added afterward; final focused ForeignEmissionTest/AggregateEmissionTest/StructEmissionTest suites pass 142 tests/configuration in `final-focused-debug/release.xml`, including that test. The two phases together cover every test in the final tree; no NativeAOT tests ran.

PASS: `backend/windows-x64/test-scalars.ps1 -FixturePattern 'ForeignPointer*.ll' -OutputDirectory bin/verification-20260922-050722/t26w-native` validates and executes 16 freshly generated fixtures, 32 O0/O2 runs with pinned LLVM 22.1.8 and the checked backend archive. Final file hashes are in `final-fixture-hashes.csv`. No language requirement changed, so SPEC.md and its owning chapters needed no edits. PLAN.md/STATUS.md retain the remaining general implementation boundaries.

### Final timed checkpoint

Stopped at 2026-09-22 06:42:54 UTC, **95m32s wall-clock elapsed** from the pre-inspection start. Time checks at 05:43:33 and 06:36:13 bracket an unexpectedly large clock advance; its external cause was not established, and no idle delay was introduced to satisfy the duration. On detecting expiry during T26w, no new unit was started. Completed that unit's required regression, boundary review and documentation, then stopped for the elapsed-duration condition. This is a partial compiler checkpoint, not completion of M11/M12 or the compiler baseline.

The final focused Release run passes all 142 tests. All 80 saved pointer-fixture files match their recorded SHA-256 hashes after the final Release tests. `git diff --check` passes; no draft files changed and no NativeAOT tests ran. No new out-of-scope findings. Exact remaining implementation and next actions have one home in PLAN.md §2; historical execution deadlines do not constrain a future request.
<a id="raw-pointer-owned-continuation-20260922"></a>
### Raw-pointer Owned continuation — 2026-09-22 07:09:24 UTC

Fresh inspection found HEAD `77bd5038` with the preceding T26s–w edits uncommitted. These changes are preserved. The requested 60-minute duration ends at 08:09:24 UTC; Milestone Programs, draft edits and NativeAOT remain excluded.

**T26x DONE — zero-stride language diagnostics.** Binding uses substituted logical storage shapes (including bases) to prove zero stride for Unit, tuples, fixed arrays and structs. Unknown/symbolic layouts are not misclassified as zero; generation still requires a concrete positive stride. Seven of eight initial diagnostic reproducers failed before the fix. Final tests cover all five pointer operations at zero displacement, generic and inherited empty storage, positive stride and symbolic array length. Debug/Release builds pass with zero warnings/errors; full suites each pass 11,159 tests; fresh pointer fixtures pass 32 pinned LLVM 22.1.8 ordinary native O0/O2 executions. Evidence: `bin/verification-20260922-070924/t26x-before.{xml,log}`, `t26x-full-{debug,release}.{xml,log}`, `t26x-native/`. An early focused run occurred before its build completed and is not completion evidence; the later full suites used completed builds. No specification change was necessary (§5.3/§21.1 already require this behavior).

**T26y DONE — Owned Non-Copy aggregate reads.** Raw tuple/array/struct reads now acquire an existing ownership temporary using its Copy/Move classification. Lowering accepts Move transfers while preserving the write restriction. The raw source is not given a Loan or an invented compiler owner; caller obligations for moved storage remain §5.2. The fresh ordinary-native fixture checks integer-field resources, nested tuple/array destruction, zero-sized destructors, call/result delivery, branch returns, deferred early-return cleanup and discarded temporaries. A negative test rejects reuse of an acquired Move. Debug/Release builds are warning-free and focused Foreign/Struct/Aggregate suites pass 161 tests each (including a zero-byte warmed Non-Copy read analysis/emission probe); the final expanded fixture passes both O0/O2 executions. Evidence: `t26y-{debug,release}.{xml,log}` and `t26y-native-final/` under the current evidence root. Non-Copy replacement and standalone strings remain unsupported at this checkpoint.

**T26z DONE — Owned aggregate replacement.** `StorePointer` explicitly consumes the acquired source Place; dataflow, Loan conflict/liveness, slot-result validation and physical lifetime flags share this transition. No fake destination owner is created. Lowering destroys the old pointee at its original address (including zero-sized destructors), then transfers the secured value. Normal assignment order remains RHS first. Native tests verify scalar-field resources, tuples, result storage, old/new destruction order, early return during indexing with secured-source cleanup, and Abort during old destruction without continuation or unwinding. Negative tests cover moved-source reuse and corrupted operation/acquisition/source/address records before output. Debug/Release builds have zero warnings/errors; full suites each pass 11,168 tests; 19 freshly generated pointer fixture sets pass 38 ordinary native O0/O2 executions. Warm analysis/emission of scalar, Copy aggregate, Non-Copy read and Non-Copy replacement workloads allocates zero bytes over each 128-iteration measured loop after 100 warmups. Evidence: `t26z-full-{debug,release}.{xml,log}`, `t26z-native-final/`. Standalone string pointees remain a separate next slice; dependent/reference pointees, subplace projections and C layout remain unsupported.

**T26aa DONE — standalone string pointees.** String Move reads/replacement use the existing three-field handle transfer and release routines. Zeroed empty Static handles are valid under §22.5.5. Inspection uncovered that a non-consuming raw access must not be silently changed into a Move: `ReadPointer` now preserves this unsupported boundary rather than giving string comparisons an owning temporary. Explicit acquired values and function-result comparisons retain ordinary ownership behavior. Direct raw string comparisons/borrows remain pending an access plan. Negative reference tests now spell a complete static Referent Origin; initially omitted nested Origins failed Binding before reaching the intended check. The first native lifetime audit omitted three required old-empty destructions and correctly failed with audit exit 121; the corrected oracle counts them. Final Debug/Release builds are warning-free; full Debug passes 11,175 tests and focused Foreign/String suites pass 144 each. Both conditional consumption paths and audited variants pass eight O0/O2 executions, with exact release counts including short-circuit skips; the warm string read/write analysis/emission probe allocates zero bytes. Evidence: `t26aa-full-debug.{xml,log}`, `t26aa-{debug,release}.{xml,log}`, `t26aa-native-final/`. The initial build's formatting warning is fixed. No source-level raw initialization or reference-conversion API was added.

**T26ab DONE — complete Owned enum pointees and layout reuse.** Raw enum reads/replacement use the same validated aggregate transfers and active-Case destruction as ordinary storage, including generic/nested payloads and 16-byte alignment. Native tests check case changes, resource destruction, string releases, nested enums, Moves through return/match and indexed u128 payloads. The new warm enum probe initially measured 10,240 bytes over 128 loops (80 bytes/loop): GetEnum allocated a case array before searching its existing layout pool. A reusable stack-scoped case list now compares spans and allocates the retained array only for a new layout, restoring zero measured allocation for both direct and nested enum workloads. Debug/Release builds are warning-free; full Debug passes 11,178 tests, focused Foreign/Enum suites pass 117 each; the final normal/audited fixtures pass four O0/O2 executions. Evidence: `t26ab-full-debug.{xml,log}`, `t26ab-{debug,release}.{xml,log}`, `t26ab-native-final/`. General enum and layout obligations remain in their parent milestones.

**T26ac DONE — checked layout metadata and concrete C storage.** A focused reproducer found `Layout`, `"C"` and `Layout("C")` pending in control-flow analysis despite completed Binding. Verified Layout attributes now remain declaration metadata instead of becoming runtime calls/string evaluations. Explicit Kimigayo layout uses the default alignment-sorted representation. Concrete C structs use written field order, checked size/offset arithmetic, ordinary LLVM structs and separate C/Kimigayo physical pool keys. Open/derived/empty C structs, direct zero-sized fields and alignments above 16 reject rather than falling back. The existing 32-bit physical-offset implementation limit remains explicit; full C formation diagnostics and C-exchangeability certification are still backlog.

The C fixture verifies offsets, alignment, stride, mixed C/Kimigayo cache shapes, nested C fields, generic instantiated fields and Non-Copy replacement/destruction. Clang 22.1.8 independently compiles Windows x64 `/Zp16`-equivalent (`-fpack-struct=16`) static assertions for the tested sizes/alignments/offsets. Earlier tests asserting rejection of valid C layout now assert its specified ordinary LLVM representation; invalid C layouts retain negative coverage. Focused Foreign/Struct/Layout suites pass 193 Debug tests, and the final broad suites cover the same source in both configurations. A new warmed C replacement probe allocates zero bytes.

**Final verification for T26x–ac:** Debug/Release solution builds pass with zero warnings/errors. `dotnet xUnitTest/bin/<configuration>/net10.0/xUnitTest.dll -parallelMode none -failSkips -result-xml ...` passes **11,186 tests per configuration**. After both fixture producers finish, `backend/windows-x64/test-scalars.ps1 -FixturePattern 'ForeignPointer*.ll' -OutputDirectory bin/verification-20260922-070924/final-pointer-native` verifies and executes **27 fixture sets / 54 O0/O2 runs**, including destruction-count audits. Evidence: `final-full-{debug,release}.{xml,log}`, `final-pointer-native/`, `c-layout-oracle.{c,obj}`, and `final-pointer-fixture-hashes.csv` under this execution's evidence root. Earlier native checks are focused diagnostic evidence; this serialized final native run is the authoritative final fixture verification.

HEAD advanced externally from `77bd5038` to `81926c3b` during execution, incorporating the implementation checkpoint. No assistant commit, reset, draft edit, Milestone Program execution or NativeAOT test was performed. Final documentation updates preserve that checkpoint. Specification text already defines the implemented behavior and was not changed. Remaining implementation and exact next actions are recorded only in PLAN §2; parent milestones are not marked complete.

**Timed checkpoint:** 2026-09-22 08:09:36 UTC, 60m12s after the recorded start. The requested duration elapsed during final T26ac verification/documentation; that unit is coherent and verified. No new unit started after the deadline. Stop reason: requested duration reached. The final hash inventory contains 135 pointer fixture/oracle files. PLAN §2 records the exact next action; no incomplete parent milestone is marked DONE.

<a id="raw-pointer-subplace-continuation-20260922"></a>
### Raw-pointer subplace continuation — 2026-09-22 08:14:16 UTC

Start time recorded before inspection (17:14:16 JST); deadline 09:14:16 UTC. Fresh inspection found HEAD `53ac0e4b`, a clean tree, and only PLAN/PLAN_HISTORY/STATUS changes after the recorded `81926c3b` checkpoint. Milestone Programs, draft edits and NativeAOT remain excluded. Evidence root: `bin/verification-20260922-081416/`.

**T26ad DONE — static raw-pointer subplace projections.** §5.2 makes `*p` a writable Place of Type `T` and §12 requires `(*pointer).name` for field access through raw pointers. A five-case reproducer (`(*p).second`, `p[1].first`, `(*p).1.0`, `(*p)[2]`, `p[1][0]`, each read then written) failed before the change: Binding rejected the Tuple/array writes with `InvalidAssignment_Kd`, and ownership read `(*p).second` by loading the whole `Pair`, then had no write plan. Binding now treats an inline stored field/Tuple/fixed-array path rooted at `*p` or `p[n]` as writable (`ElementAccess.IsPointerPath`), independent of binding mutability. Ownership treats stored-field, Tuple-element and literal in-range array levels as raw Places: each level is a `PointerProject` value of the canonical `unsafe/Part` Type (`Binding.PointerType`, no Origin), and the existing pointer load/store/replacement plans then access only that part. No Loan or compiler owner is created, the rest of the pointee is not read, and a Non-Copy container is not moved to access a Copy or string field. Lowering validates the operation, dominance, container/part Types and layout position, and writes `getelementptr i8, ptr base, i64 offset` without `inbounds`. Computed array indices below a raw Place keep the unsupported boundary (they need a checked bounds plan).

`ForeignPointerSubplace` checks Kimigayo and C struct field offsets (including raw byte inspection of the C `tail` field), `p[n].field`, nested Tuple elements, literal array elements, compound updates, increments, one evaluation of the pointer operand, and a Non-Copy `Holder` whose `count` is read and updated and whose `label` is replaced and then moved. Three negative cases confirm computed indices below raw Places complete Binding but publish no IR. A warm workload with nested struct/Tuple/array projections allocates 0 bytes over 128 iterations after 100 warmups.

Boundary probe (not retained as a test): explicit `@ref` of `*p` or `(*p).1` stays rejected by Binding. An implicit shared argument borrow such as `look((*p).1)` with `look(x: ref/i32)` binds as a §10.2 owner-temporary borrow. It now materializes a Copy temporary exactly like the pre-existing `look(*q)` for `q: unsafe/i32`; before this change it failed lowering validation ("Element address is not protected by its receiver Loan"). No safe reference to raw storage is created. Raw-to-safe reference conversion remains unspecified (§5.6).

PASS: Debug solution build and full Debug suite, **11,191 tests** (`t26ad-full-debug.{xml,log}`); 13 focused cases. PASS: `backend/windows-x64/test-scalars.ps1 -FixturePattern 'ForeignPointerSubplace*.ll' -OutputDirectory bin/verification-20260922-081416/t26ad-native`, a freshly generated fixture with 2 LLVM 22.1.8 O0/O2 executions. Release: a Release build/full run passed 11,191 tests with zero warnings, but it may have overlapped the first T26ae source edits, so it is not cited as T26ad evidence; the final verification below covers the final tree. Specification text already required this behavior and was not changed; STATUS.md records the new boundary.

**T26ae DONE — bounds-checked computed indices below raw Places.** §4 fixed-array indexing is checked against the length independent of how the array Place is reached; §5.3 raw pointer indexing (`p[n]`) itself stays unchecked. Computed `isize` indices at fixed-array levels of a raw path are now raw Places: the index is evaluated after the container address, and `PointerProject` carries `[container, index]` (Constant -1). Lowering now emits every projection through the existing `ElementAddress` opcode: static levels as an unchecked constant offset, computed levels with `ArithmeticCheckKind.Bounds` (unsigned compare, so negative indices fail too), a source location and the element stride. `Operand` maps a projection value to that element address. This replaced the T26ad one-operand `PointerOffset` writer case, which was removed. T26ad's three "computed index remains unsupported" negative cases were superseded by positive coverage. Range and from-end forms keep their existing handling; Binding rejects from-end indexing for fixed arrays in general (`UnsupportedBinding_Kd`), so no raw-specific test was kept for it.

`ForeignPointerSubplaceIndex` verifies plain assignment order (RHS, pointer operand, index), compound order (pointer operand, index, RHS), nested computed indices in loops, prefix increment, and whole-value readback. `ForeignPointerSubplaceBounds{High,Negative}` check the exact `KIMI_E_INDEX_BOUNDS` abort, location and exit code. A four-case corrupted-plan test (wrong static position, wrong computed marker, wrong container address, non-isize index input) rejects before writing any IR. The container case initially passed validation: the rewritten lowering had dropped the container-Type check present in T26ad. That check was restored; the test was not weakened. The first native run failed because the test's expected stdout omitted one `index` line printed by the program; the program's order is the specified one, and only the oracle was corrected.

PASS: focused Debug `ForeignEmissionTest`/`StructEmissionTest`/`AggregateEmissionTest` 194 tests (`t26ae-focused-debug.xml`); after the oracle fix, `ForeignEmissionTest` 121. PASS: `test-scalars.ps1 -FixturePattern 'ForeignPointer*.ll' -OutputDirectory bin/verification-20260922-081416/t26ae-native`, **31 freshly generated fixture sets / 62 LLVM 22.1.8 O0/O2 executions**, including both bounds aborts.

**T26af DONE — in-place raw string comparisons.** T26aa deliberately rejected `*p == "text"` rather than turning a non-consuming inspection into a Move. String comparison operands that are raw Places (`*p`, `p[n]`, and subplaces such as `(*h).label`) now record their handle address as the comparison's value with Place -1. No temporary owner, Move, Loan or cleanup is created. Lowering accepts that form only for a dominating `unsafe/string` address with no Loan, and passes the address to the existing `StringEquals`/`StringCompare` runtime. `InspectString` keeps the prior `ref/string` value rule on both its element and ordinary paths. `StringPointerComparisonsDoNotInventAnOwningRead` previously asserted the unsupported boundary. It now asserts verified ownership, no `PointerLoad` and no string Place for the raw operand, preserving its purpose: no owning read is invented. `ForeignPointerStringCompare` checks `==`, `!=`, ordering in both operand positions and a Non-Copy struct field, then moves each value out. Its audited variant counts exact releases (`=3;alpha=3;beta=2;kept=2;done=1`): comparisons add none. A warm raw-comparison workload allocates 0 bytes.

PASS: focused Debug Foreign/String/Struct/Aggregate suites 238 tests; warm probes 10. PASS: `test-scalars.ps1` with fresh fixtures: `ForeignPointer*.ll` 33 sets / **66** O0/O2 executions (`t26af-native-pointer`) and, because every string comparison now shares the refactored inspection, `String*.ll` **478** executions (`t26af-native-string`).

**T26ag DONE — §21.1 C-layout formation diagnostics.** §21.1 says C layout "is rejected" on open or derived structs, empty structs and direct zero-sized Fields. Previously only split storage was diagnosed; the other forms failed only in generation, and only when a body reached the struct. Binding now reports `InvalidCLayout_Kd` (new code and message) on the selecting Attribute for open, derived and empty structs. Static storage does not count as an instance Field. The same code is reported on each direct instance Field whose concrete Type is proven zero-sized (Unit, zero-length arrays, arrays of zero-sized elements, empty structs). That Field check runs after `PrepareStorage`, because struct size proofs need storage shapes; a first placement before storage preparation missed the empty-struct case and was moved. The container also receives `InvalidTypeFormation`. Fields whose size depends on a Type argument keep the generation-time rejection. The C struct list is a reused Binding field. Two existing tests (`LayoutCertificateBindingTest.RemovingConflictRestoresConformanceAfterRebind`, `LibraryImportTargetBindingTest.RemovingImportFromChainRestoresConformance`) used empty C structs incidentally and began failing correctly. Each gained one `i32` Field; their conflict/rebind and import-removal assertions are unchanged. Eight new diagnostic cases and two positive cases (static plus instance Field; generic `T` Field) were added.

PASS: Debug solution build with zero warnings; full Debug suite **11,207 tests** (`t26ag-full-debug.{xml,log}`). Release: see the timed checkpoint below. No native fixture changed: this unit adds declaration diagnostics only, and the existing C-layout native fixture still generates in the full suites.

**T26ah DONE — written C-layout instantiations.** §21.1's example states that C-layout `Pair<()>` violates the zero-sized Field restriction. `BindConstructedType` now checks a written application of a struct whose Attribute chain selects C layout. If the instantiation makes any stored Field zero-sized (the same storage shape and substituted zero-stride proof as T26x), `InvalidCLayout_Kd` is reported at the application syntax. Header Types bind before `PrepareStorage`, so their applications are recorded and checked once storage is prepared; body Types are checked immediately. The deferral list and flag are reused Binding fields reset at the start of each Bind. The C mode is read from the Attribute chain rather than the post-header C struct list, which is not yet populated when headers bind. Tests cover parameter, local `unsafe/` and empty-struct arguments, two valid instantiations, and identical results after rebinding. Inferred instantiations without written Type syntax keep the generation-time rejection.

**Final verification for T26ad–ah (final tree):** Debug and Release solution builds have zero warnings/errors. `dotnet xUnitTest/bin/<configuration>/net10.0/xUnitTest.dll -parallelMode none -failSkips -result-xml ...` passes **11,212 tests per configuration** (`t26ah-full-{Debug,Release}.{xml,log}`, `t26ah-build-*.log`). No source edit occurred during these runs. A final behavior-preserving cleanup then moved `LowerPointerProjection`'s stride-availability check before its instruction is appended. It was reverified with warning-free builds and 11,212 tests per configuration (`final3-full-{Debug,Release}.{xml,log}`), followed by the native run below. Then `test-scalars.ps1 -FixturePattern 'ForeignPointer*.ll' -OutputDirectory bin/verification-20260922-081416/final-native-pointer` passes **33 fixture sets / 66 O0/O2 executions** on fixtures regenerated by the final Release suite, including string-release audits and bounds aborts. Hashes are in `final-pointer-fixture-hashes.csv`. The `String*.ll` native run (478 executions) is from T26af; later units changed only Binding declaration checks. Earlier intermediate Release runs that may have overlapped source edits are superseded by this run. `git diff --check` passes. No draft edit, commit, Milestone Program or NativeAOT test.

**Timed checkpoint:** stopped at 2026-09-22 09:06 UTC, about 52 minutes after the recorded 08:14:16 UTC start. T26ad–ah and the cleanup are complete and verified. The roughly 8 minutes remaining could not hold another verifiable unit: the next planned item (I28 combined-module native records) needs a link-manifest schema revision, and full Debug/Release verification alone takes about 4 minutes. Its exact steps are in PLAN §2. No parent milestone is marked complete. No draft edit, commit, Milestone Program or NativeAOT test.
