...
1. **P20 (next milestone):** run the Program 20 build probe with the current compiler; the recorded blocker is `UnsupportedBinding_Kd` when dereferencing generic returned element borrows (README program 20). Start with that Binding shape, then owned/borrowed defaults (§7.2) and forwarding.
