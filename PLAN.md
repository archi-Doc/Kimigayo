...
- **Verified source HEAD** `ad39b8cc`. Debug/Release builds are warning-free; each full suite passes 12,169 tests. Session evidence: `bin/verify/20260924-004048-session-p30-session1`; 48 Comparison native O0/O2 executions and all 24 completed harnesses (1–22, 29 and 32; Release; 1,232 checks) pass. P30 remains IN_PROGRESS; P29 and P32 are DONE.
...
- **Programs 1–22, 29 and 32** have verified native execution. Programs 23–28 and 30 are authored targets with failed build probes; 31 and 33–38 are not written yet. Source creation or a successful target run alone does not complete an implementation milestone.
...
| 7 | P30 | Equatable/Comparable Contracts | IN_PROGRESS | Primitive/user/generic requirement calls and operators pass; Tuple/shared-borrow composition, Copy aggregate referent reads and the complete Program 30 harness remain. |
...
1. **P30 composition (G18):** implement recursive Equatable/Comparable conformance and witnesses for shared borrows and Tuples, preserving NaN-reflexive Contract equality separately from built-in IEEE Tuple equality; keep Program 30 unchanged.
2. **P30 Copy reads (G19/G11):** create a real aggregate referent snapshot before borrowing it for a comparison witness; verify evaluation order, Loan acceptance and cleanup.
3. **P30 completion:** run the unchanged Program 30 and all harness variants/rejections in Debug/Release at O0/O2, measure composed comparison costs, then check every §5 condition before marking DONE. Broader Array/Slice/iteration follow-ups remain with P27/P28 and G14.
...
| G18 | Program 30 is authored with user approval (2026-09-24); its Tuple/shared-borrow Contract composition is not implemented. Keep the target unchanged. Add recursive conformance/witness plans for borrowed and Tuple elements, retaining NaN-reflexive Contract equality separately from built-in Tuple operators, then run the complete harness. Primitive/user comparison work and the independent rejection harness can progress separately. | P30 |
| G19 | A concrete `ref/Key` comparison where `Key is Copy, Comparable` binds but reaches the existing unsupported aggregate referent-read path (`OwnershipAnalysis.LoadReferent`, related to G11). Reproducer and failed positive test: `bin/verify/20260924-003445-unit-p30-copy-identity/tests-Debug-focused.log`. The partial test was reverted; the specification is unchanged. Implement a real Copy snapshot before borrowing it for the witness, preserving left-to-right evaluation and Loan acceptance; do not replace the read with an original-storage borrow. | P30/P23 |
