...
3. **Places and iteration syntax:** parse and bind `place(ref|uniq, T)` results, Contract generic parameters, `associate Name(params)`, `wellformed` clauses and `T.(C).LentItem(a)`, then implement the user `Iterable`/`UniqIterable`/`IntoIterable` protocols and the `Indexable` family within P27/P28. Until then these forms stop at the parser (STATUS records the boundary).
