...
- **Verified source HEAD** `3e5aafeb`. Debug/Release builds are warning-free; each full suite passes 11,363 tests. Session evidence: `bin/verify/20260922-162013-session-p22-shared-writer-removed`; all existing harnesses (1–12 and 14–19) and 168 generic native O0/O2 executions pass.
...
1. **P22 walk-to-validator:** turn `GenericStoragePlan.PrepareBody` into a pure validator: keep every per-operation invariant check it performs (the corrupt-plan regressions in `GenericForwardingEmissionTest`, `GenericCallbackEmissionTest`, `GenericStorageEmissionTest`, `LengthStorageEmissionTest`, `SharedAbortEmissionTest` and `PipelineCallableTest` rely on them) but drop the `SharedStorage*` instruction/leaf/policy construction, then delete the `SharedStorage*` records, `SharedStorageOperation`, the unused helpers (`StorageValue`, `IsSharedScalar`, `SharedScalarType`, `ComparisonPredicate`) and reduce `Template`/`SharedStorageEntry` to `Body`/`DirectCalls`/`ConcreteOnly` and `Abi`/`Parameters`/`Result`/`Selected`. Drop the walk's shared-only refusals so direct result forwarding (`forward<T>(value: T) -> T => identity<T>(value)`) becomes an instance shape (flip `RejectsUnsupportedDependentResultForwarding` to a native fixture). Rename `SharedEntries` to pending entries.
2. **P22 program:** with the walk retired, resolve `milestones/Milestone22.kimi`'s remaining generation failure and add its harness (see 3).
