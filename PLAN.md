...
- **P31 IN_PROGRESS:** the unchanged target and all 59 Debug/Release harness checks pass, as do generic/zero-sized entries, insertion-order iteration, slot reuse and growth/allocation bounds. Nonempty literals and broader borrowed/nested storage forms remain.
...
| 8 | P31 | Dictionary | IN_PROGRESS | Unchanged target and 59 Debug/Release harness checks pass. Empty storage, all seven APIs, generic/zero-sized entries, owned indexing, shared/owning iteration, cleanup and allocation/cost guarantees are verified. Nonempty literals, borrowed indexing and nested owning storage remain. |
...
1. **Finish P31:** nonempty literals must diagnose mandatory duplicates and check runtime duplicates before value evaluation. Extend borrowed-handle indexing and nested owning storage through the existing acquisition/storage plans; add the corresponding harness cases.
2. **P23/P24:** complete custom/computed accessors and ownership-bearing getters/setters with the shared callback, acquisition and replacement plans.
3. **Continue the milestone order:** finish P25 inherited projections/implicit base calls, then the planned closure/Slice/Iterator work and P33 flow refinement. Preserve immutable object identity and invalidate content facts on complete payload updates. General ObjectCallCompatible remains deferred.
