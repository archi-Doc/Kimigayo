...
| 8 | P31 | Dictionary | IN_PROGRESS | Empty handles, metadata, reserve, owned transfer, search/mutation APIs, lookup Loans, reverse insertion cleanup and allocation-free churn execute natively. Generic/zero-sized entries and failure-preserving shrink also pass. Shared/owning pair iteration, external Origins and early-exit cleanup pass. Owned indexed replacement/updates and the unchanged target now pass Debug O0/O2. The dedicated harness passes 59 Debug checks. Nonempty literals, borrowed indexing, nested storage and Release/session completion remain. |
...
| G14 | Shared Tuple iteration now Copies complete Copy components (including stored references) and borrows Non-Copy owned components with retained Origins. String reference locals/results and fixed-array/Array/Slice iteration now pass; shared literal/string and Tuple/enum patterns now pass with retained nested reference paths; shared/composite guard candidates now pass with cleanup-spanning protection and retained Copy Origins; stored exclusive references and unique object handles use shared reborrows; concrete Array/Slice indexing shares the same acquisition rules; unknown correlated shared-read families and broader general sequence operations remain; use the common shared-read acquisition rule when extending these forms. | P28/P29 |
