...
Current position: **M11/M12 — I26/I28 foreign import calls, root-module supplies and raw-pointer values**. T26j, T28g, T26k, T26l, T26m, T28h, T26n and T26o (2026-09-22) are DONE,, T26p binds dereference and T26q/T26r generate Copy scalar/pointer reads and writes: valid `#LibraryImport` declarations complete Binding; direct calls with integer/`f32`/`f64`/raw-pointer signatures lower to one shared external declaration per symbol (`dllimport` for `import` supplies; reserved kernel32 runtime declarations are shared); publication lists the root project's self-targeted supplies required by its imports in the link manifest, and `build` links hash-named staged snapshots of them after checking their Kind/Sha256 assertions, admitting only declared foreign object references (quoted names included). Raw pointers are Copy `ptr` values with `null`, same-Type `==`/`!=` (§5.1), `@` conversions to other pointer Types and `usize` (§5.4–5.5) and `p ± n`/`p ±= n` displacement (§5.3). Kernel32 imports (fixtures, including pointer-returning `VirtualAlloc`) and a Clang-built static supply (CLI script) execute at O0/O2. Next: §2 next action 1. [Evidence](PLAN_HISTORY.md#foreign-import-calls-20260922).
...
| M11 | IN_PROGRESS | T26a–i validate `#LibraryImport` declarations; T26j–o generate direct import calls (scalar, float and pointer signatures, quoted symbols), raw-pointer `null`/equality/conversions/displacement, and link root-project supplies (with T28g/h). Dereference, indexing, dependency-module supplies, provider identity and C layout execution remain. |
...
1. I26/I28: T26j/T28g resolved the 2026-09-21 probe blockers and link the root project's own supplies. T26k/T26l added raw-pointer values and conversions. T26n/T26o added displacement. Next, in order: dereference and indexing (§5.2–5.3). T26p binds `*p` (writes permitted regardless of binding mutability) and T26q/T26r lower Copy scalar/pointer reads and writes. Add: compound assignment through pointers, bool and aggregate pointees, Binding of `p[n]` as `*(p + n)`; ownership read/write operations on a pointer-addressed Place with no Loan and Copy-only reads first (a Non-Copy Move leaves the programmer-managed storage uninitialized); lowering `load`/`store` through the pointer operand with the pointee alignment and no invented attributes; a native fixture writing and reading `VirtualAlloc` memory; combined-module native records so imports defined in dependency modules connect their scoped requirement to its supply (§20.8.3, §18.5.2) instead of failing publication; archive member indexing for actual Kind, provider identity and ambiguity (§20.8.2.3). I11 dynamic static initialization/shutdown is independent; I6 retains general Origins, joins and backedge work.
