...
Current execution: **General compiler implementation — timed (60 minutes)**, started 2026-09-18 15:31:30 UTC (2026-09-19 00:31:30 JST) from clean HEAD `f31d2ba`. Earlier timed checkpoints (T4r, T6a–T6c) retain their bounded completion; the compiler Milestones remain incomplete. Documentation Comments retain their completed checkpoint and the adopted design's precedence. [Execution evidence](PLAN_HISTORY.md#general-compiler-20260918-153130); [previous execution](PLAN_HISTORY.md#general-compiler-20260918-142715).
...
The 60-minute execution stopped after T6j; see the [stop record](PLAN_HISTORY.md#general-compiler-20260918-153130).

Next unit T6i (M3/I6) TODO: support explicit borrows of owned-local inline subfields into locals (`var pair = Pair.init()` then `let a = pair.left@uniq` / `@ref`), currently `Unsupported` at `pair.left` in `BorrowStruct` because an owned-Left MemberAccess falls to a whole-value read. Reuse the existing owned element-projection Loan machinery (`ElementAccess.IsSyntax`, `Projections`, `ElementPathsOverlap`) rather than a parallel path; lowering needs a slot address plus the validated inline offset. Keep sibling fields disjoint and reject overlapping parent use, Moves and replacement while the child lives. Verify with focused managed, full Debug/Release and fresh native O0/O2 fixtures. General contextual nested calls/defaults, shared generic updates and wider M/I criteria remain open; independent executable items remain authorized if these are blocked.
