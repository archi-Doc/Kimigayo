...
- **Verified source HEAD** `9038c638`. Debug/Release builds are warning-free; each full suite passes 11,493 tests. Session evidence: `bin/verify/20260923-030335-session-p29-session3`; all 21 harnesses (1–20 and 22, Release, 1,011 checks) pass.
...
| 5 | P29 | Dynamic Array | IN_PROGRESS | §4.7 capacity, mutation, Non-Copy elements, owning iteration and mandatory allocation bounds; settles issue G3. Program 29 and its README are authored; `Array<T>` binds, and empty handles, metadata, borrowed handles and value passing generate and run natively; element storage, the mutation operations, indexing and iteration are pending. |
...
1. **P29 (in progress):** next unit is the mutation operations: declare `reserve`, `append`, `insert`, `pop`, `remove`, `clear` and `shrinkToFit` as bodyless signatures in a new `Kimi/Library/ArrayOperations.kimi` read with `signatures: true` into the `Array` struct (`KimiLibrarySources.ParseSource` appends parsed `func` declarations to its container; add a struct-scoped `KimiLibraryContainer.Array` whose scope owner is the loaded `Array` declaration), register them as catalog compiler functions (`KimiDeclarationId`/`CompilerFunctionKind` entries validated like writeLine, with `self: uniq/Self` receivers and `T` inputs; confirm member lookup reuses the catalog `BoundSymbol`), add `__kimi_array_grow` (checked byte size, amortized growth, HeapAlloc/copy/free) to WindowsRuntime.ll.in, and lower each operation as a runtime call with element transfer into `buffer + length * stride` (SPEC 4.7.2, 4.7.4, 4.7.6; destruction of elements in reverse index order before the buffer is released for cleanup-bearing T). Then the `Index` Type for `^n` (parser: FromEndIndexKoto), element literals, indexing, owning iteration and `test-milestone29.ps1` (issue G3).
