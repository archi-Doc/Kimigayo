...
- **P27 IN_PROGRESS:** Program 27 stops at Binding (`UnresolvedBinding_Kd` at `Range`). Slicing with half-open `isize` ranges, Slice element reads and borrows, `indices`/`length`/`isEmpty` and Slice iteration are verified; `Range` values, `Index`/`ResolvedRange` keys, `resolve`/`tryResolve`, the try-prefixed Slice operations, `splitAt`, generic reslicing with Origin equalities, nested views and the Place foundation remain (§6).
...
| 15 | P37 | Integrated processing application | TODO | Collections, borrows, iteration and closures combined. |
...
| 19 | P34 | Shared object ownership (rc/arc) | TODO | Validate source declarations, Binding, ownership and runtime support for the already-cataloged ownership family (issue G4). |
| 20 | P35 | Weak | TODO | Downgrade/upgrade, expiration, cyclic construction, table release. |
| 21 | P36 | General static storage | TODO | First initialization, effects, cycles, shutdown. |
| 22 | P38 | Integrated core application | TODO | Properties, inheritance, objects and formatting combined. |

P22, P19, P20, P21, P29, P32, P30 and P23 are done. P27 is the active milestone.
...
1. **P27 Place foundation:** parse `place ref/T`/`place uniq/T` results and Contract Type parameters; bind Place-returning functions (the return operand designates a Place, `during self`), calls whose result is a Place (acquired like an element Place: bare Copy, `@ref`/`@uniq`, implicit borrow at expected references, assignment target, no Take), the `Indexable<Key>`/`UniqIndexable<Key>` Contracts and `receiver[key]` resolution with `index`/`indexUniq` selection from the use; lower Place results with the reference ABI (§21.2). Extend Program 27 with `Pair<T>` and the generic `firstPlace` once the parser accepts the forms.
2. **P27 Slice/Index/Range:** `Range` as a Kimigayo struct constructed by range syntax (`Index` boundaries, inclusive flag, Equatable), `Index.resolve`/`tryResolve` and `Range.resolve`/`tryResolve` in Kimigayo, `ResolvedRange.init`, `Index`/`ResolvedRange`/inclusive-range keys, `tryGet`/`trySlice`/`splitAt`/`trySplitAt`, nested fixed-array views, generic reslicing with `origin result.source == values.source`, inner-reference element Copy, Non-Copy element borrows at `ref` parameters, the §4.6.4 evaluation and Abort order, and the §4.6.8 O(1) cost evidence.
3. **P27 completion:** `test-milestone27.ps1` with the README variants and rejections, STATUS boundary, session verification.
4. **P39 decision:** review `draft/Design/2026-09-26 Semantics-Generic Follow.md`; on acceptance integrate it into §3.4.1, §8.1.1, §8.9 and §13.5.5.1, author Program 39 and implement it before P28.
5. **Then P28, P31, P40, P26, P37** in the order of §4; the Property/object track follows.
