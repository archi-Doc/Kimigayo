...
Current focused checkpoint: **DM4 — benchmarks and measured improvements complete for the implemented parser API**. Baseline `4c53331`. Same-process comparison across six representative inputs reduces geometric-mean parse time by 36.0% and allocations by 51.8%; final Markdig comparison and limits are recorded in the [benchmark report](Benchmark/DocumentationMarkdown.md). Debug/Release solution builds are warning-free; all 10,175 managed tests, including 965 documentation tests, pass per configuration. The existing Markdig-backed `DocumentationMarkdown` API remains the product path. [Execution evidence](PLAN_HISTORY.md#documentation-markdown-benchmarks-20260920); general compiler items below remain backlog.
