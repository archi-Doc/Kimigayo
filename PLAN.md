...
- **Open regression (2026-09-27):** the session run at `f5a1ec21` fails one test in each full suite, `PatternBindingTest.ReplacingPatternRebuildsScopeAndRetiresRemovedMatch` (a replaced Pattern's old binding `n` still resolves on rebind). Bisected to `bef399c5` (Range values); the Release harnesses of that session did not run. Fix before the next unit ([history](PLAN_HISTORY.md#programs34-39-authoring)).
...
0. **Regression fix:** make `ReplacingPatternRebuildsScopeAndRetiresRemovedMatch` pass again (introduced by `bef399c5`; compare the arm-scope reset with the synthesized Range/Index constructions), then rerun the session verification with the Release harnesses of Programs 1–23, 27, 29–32 and 37.
