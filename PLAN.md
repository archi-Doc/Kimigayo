...
- **Verified source HEAD** `2ab610ad`. Debug/Release builds are warning-free; each full suite passes 11,374 tests. Session evidence: `bin/verify/20260922-170925-session-p20-scalar-borrow-comparison`; all existing harnesses (1–12, 14–19 and 22) and 30 scalar-borrow native O0/O2 executions pass.
- **P20 progress:** safe scalar borrows compare their referents (§13.4) and literals under `@ref` fit an expected referent (§10.8). Program 20 binds only with its checks rewritten (issue G12); its ownership analysis then fails the completion assertion recorded in §6.1.
...
- **Programs 1–19 and 22** have verified native execution. P22 is complete: unchanged source (SHA-256 `F65CC4AC23ED1476157F5A86F4DC850081BCE8B5E72609D267180582F9CFF280`), Debug/Release O0/O2, 45 harness checks per configuration. **Programs 20–21** retain their failed Binding probes. Programs 23–24 are authored targets with prior failed build probes; 25–38 are not written yet. Source creation does not complete an implementation milestone.
- **P22 progress:** each concrete call context of a verified generic body is re-analyzed under its substitution (`OwnershipAnalysis.AnalyzeInstance`) and lowered by the ordinary `BodyLowering` under the caller-facing entry ABI (`LlvmEmitter.LowerInstances`). Every generic shape in the suites and Programs 8–11, 13, 14, 18 and 19 now monomorphizes (a full-suite probe found no refused instance); selected explicit specializations are called directly, and a refused instance fails generation instead of falling back. Growing keys, the depth bound and more than 1024 distinct contexts per body issue `GenerationResourceLimit_Kd` (§21.3.5). The shared storage writer and shared entry planning are deleted; `GenericStoragePlan.PrepareBody` (the shared walk, with `GenericStoragePlan.Match` and `BodyLowering.Shared`) survives only as the template validation whose result is discarded.
...
| 3 | P20 | Type/length/Origin inference, defaults, forwarding | IN_PROGRESS | Returned element borrows are compared as borrows (§13.4; the authored safe `*` is issue G12); owned/borrowed defaults per §7.2; the completion assertion of §6.1 resolved. |
...
P22 (generation migration) and P19 (completed first at the user's direction) are done; the next milestone in order is P20. Collections, comparison and formatting (P29–P32) precede Properties and objects because most later programs use them.
...
1. **P20 (next milestone):** with its checks written as `inferred == 10@ref` (issue G12), the whole Program 20 source binds (`GenericBorrowedElementTest.BindsProgram20WithReferentComparisons`), but ownership analysis then fails the Debug assertion "Control-flow completion and ownership runtime predecessors disagree" (`OwnershipBody.AssertCompletion`), also when every check is replaced by `true`, so the fault is in the program's other shapes: bisect the `do` blocks (omitted default `index: isize = defaultIndex()` evaluated at generic call sites, `forward` propagating `ref{source}/T` results, the explicit `index: 2`), starting with a single `do` block and one call. Then owned/borrowed defaults (§7.2), the README rejections and the harness.
2. **P22 cleanup (any session):** turn `GenericStoragePlan.PrepareBody` into a pure validator keeping its per-operation invariant checks (the corrupt-plan regressions in `GenericForwardingEmissionTest`, `GenericCallbackEmissionTest`, `GenericStorageEmissionTest`, `LengthStorageEmissionTest`, `SharedAbortEmissionTest` and `PipelineCallableTest` rely on them), then delete the `SharedStorage*` records, `SharedStorageOperation`, the unused helpers (`StorageValue`, `IsSharedScalar`, `SharedScalarType`, `ComparisonPredicate`), reduce `Template`/`SharedStorageEntry` to `Body`/`DirectCalls` and `Abi`/`Parameters`/`Result`/`Selected`, and rename `SharedEntries` to pending entries. Add a diagnostic for invalid infinitely recursive inline layouts (README program 22 separate check).
3. **P23/P24 programs:** `milestones/Milestone23.kimi` and `Milestone24.kimi` and their README expectations are authored; implement their Property operations and harnesses at P23/P24.
...
| G12 | `milestones/Milestone20.kimi` reads its returned element borrows with `*inferred == 10`; SPEC §3.3 and §13.2 define `*` for raw pointers only and add no safe dereference (`GenericBorrowedElementTest.RejectsSafeDereferenceInProgram20`: five `TypeMismatch_Kd`). Everything else in the program binds. Proposed decision: change the program's five checks to compare borrows per §13.4 (`inferred == 10@ref`, implemented as the scalar-borrow comparison unit) or approve a SPEC addition of a safe Copy dereference; the user decides. | P20 |
