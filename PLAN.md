...
| ORD4 | MANAGED_VERIFIED — native unavailable | Library and affected milestone/test sources migrated. Debug/Release builds are warning-free and each passes 10,889 tests, including bounded allocation checks and checked IR generation. Install/configure the required LLVM 22.1.8 Windows toolchain, then run the three OriginRedesign fixtures at O0/O2 and the affected milestone suites. [Evidence](PLAN_HISTORY.md#origin-redesign-20260921). |
...
ORD1/ORD2 and the supported ORD3 paths are integrated. ORD4's remaining native verification needs the LLVM 22.1.8 Windows toolchain: run `backend/windows-x64/test-scalars.ps1 -FixturePattern 'OriginRedesign*.ll' -OutputDirectory bin/origin-redesign-native`, then the affected milestone suites. General Origin-dependent implementation continues under OSE3; do not treat managed regression as completion of those broader contracts. The following compiler-wide actions remain backlog.
