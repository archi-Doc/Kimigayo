...
The current request (2026-09-21) targets completion of `milestones/Milestone17.kimi`, including native execution, required rejection and regressions. Other programs inform prerequisite design only; stop after Program 17. Program numbers and P17 task IDs are distinct from implementation Milestones M1–M15 and I/T backlog IDs. The finalized Origin redesign remains integrated. Draft edits and NativeAOT tests remain excluded.
...
Current position: **Program 17 COMPLETE**. Debug/Release builds and all 10,961 managed tests per configuration pass. The unchanged target and its variants/rejections pass 57 native checks per configuration; programs 1–16 pass 96 Release O0/O2 regression checks. All 323 related native fixtures pass at O0/O2 (646 executions). The [final audit](bin/milestone17-work/verification.json) confirms source/compiler identities and byte-identical fixture generation across configurations. [Failures, fixes and reproduction](PLAN_HISTORY.md#program17-completion). Stop at Program 17; no later program implementation is authorized by this request.

| Item | State | Acceptance / exact next action |
| --- | --- | --- |
| P17-O | DONE | Static fixed-array paths use in-place borrows and existing completeness/Loan checks; immutable element targets cannot fall back to a temporary borrow. Positive/negative tests pass. |
| P17-G | DONE | Static path offsets feed existing update lowering; match storage pruning retains the dedicated destructor receiver. Unchanged target and variants verify, link and execute correctly at O0/O2. |
| P17-V | DONE | All required builds, managed suites, target/variant/rejection checks, pinned LLVM verification and native regressions pass. Exact output, exit, cleanup and identities are audited. NativeAOT NOT_RUN as instructed. No remaining action for this target. |
...
Programs 1–17 have bounded completion. Programs 18–21 informed design only. General M/I obligations remain open as compiler backlog.
...
Programs 12–16 retain their recorded target completion; P17 above records the latest target. Program numbers are independent of M1–M15; earlier references to M1–M17 are historical numbering errors, not two missing milestones.

### Current next action

P17-O, P17-G and P17-V are complete. Stop here; do not start Program 18. ORD4 is complete for its bounded paths; OSE3 and the following compiler-wide actions remain backlog and are not current execution instructions.

1. I26 probe (2026-09-21, reverted, no code retained): marking a validated `#LibraryImport` attribute Resolved in `ValidateLibraryImports` only moves the failure to `UnsupportedBinding_Kd` on the attribute operand and its string arguments, which are never bound; import callees also have no `OwnershipBody`, so `LlvmEmitter.TryPrepare` never registers an ABI and `LowerCall` would report "Call target has no selected implementation ABI". The slice therefore needs, together: operand binding, the caller's unsafe-call form and ownership treatment of a bodyless callee, an external-declaration record in `EmissionModule`/`LlvmModuleWriter` (`declare [dllimport]` from the §22.3.2 physical signature, registered in the function-ABI map under the external symbol), and manifest `libraries` entries for required supplies (`NativeToolchain` already links any `libraries[].input`). Then continue I26 with direct-call lowering (physical signature shared between declare and call, `dllimport`/static declaration form, >4-argument Windows x64 ABI) of `#LibraryImport` functions, then an ordinary native test against a static `.lib` supply; this also provides the first real native requirement for I28 connection validation. Provider identity beyond the completed T26g kind agreement needs resolved supplies, so it belongs with that lowering/link work rather than Binding. I28 T28d is done; next in I28, add module-scoped native input records (per-module requirements and the supplies that satisfy them) to the link manifest, which the I26 link work then consumes. I11 dynamic static initialization/shutdown is independent; I6 retains general Origins, joins and backedge work.
