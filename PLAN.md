...
- **Verified source HEAD** `1b1fd121`. Debug/Release builds are warning-free; each full suite passes 11,391 tests. Session evidence: `bin/verify/20260922-224227-session-p20-completion`; all 21 harnesses (1–20 and 22, Release) and the reference-read native O0/O2 executions pass.
...
1. **P21 (next milestone):** explicit full specialization: length specialization and inherited default/Origin contracts without cascading diagnostics. The README program 21 row records the failed O2 probe (length specialization / inherited Origin Binding unsupported); start with a Binding probe of `milestones/Milestone21.kimi`.
2. **P22 cleanup (any session):** delete the transitional walk (`GenericStoragePlan.PrepareBody`, `GenericStoragePlan.Match`, `BodyLowering.Shared`, the `SharedStorage*` records and the helpers `StorageValue`, `IsSharedScalar`, `SharedScalarType`, `ComparisonPredicate`), keep only template/entry bookkeeping and the resource limits, rename `SharedEntries` to pending entries, and re-target the corrupt-plan regressions (`GenericForwardingEmissionTest`, `GenericCallbackEmissionTest`, `GenericStorageEmissionTest`, `LengthStorageEmissionTest`, `SharedAbortEmissionTest`, `PipelineCallableTest`) to ordinary bodies, since `BodyLowering` now validates every instance. Then: generic entry ABIs from `FunctionAbiPool`; Origin-erased storage identity in lowering (drop the `instance` parameter of `ReferenceTypes.CallTypeMatches`); the instance's function name and type arguments in refusal messages; `InvalidInlineLayout_Kd` for self-containing structs and `GenerationResourceLimit_Kd` at the layout depth bound (README program 22 separate check); parallel milestone harnesses in `verify.ps1`.
5. **Copy read and default follow-ups:** aggregate Copy referent reads (a storage copy through a borrowed aggregate, currently `UnsupportedOwnership_Kd`), default calls with receivers, borrowed arguments or nested omitted defaults, and the Loan-conflict diagnostic cascade (G13).
