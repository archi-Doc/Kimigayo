...
| I18 | M7 / R7, R14–R15 | IN_PROGRESS | Checked common calls, inline Copy-environment erasure and bounded concrete exclusive Callable witnesses execute. OSE checks expected signatures and complete Origins for single nongeneric receiverless function references; wider selection shapes reject. Next: complete overload/generic/member selection, function-item environment/Origin evidence and erasure, owner/Origin-polymorphic witnesses and indirect-call ABI. T18a unsafe-value rejections remain. |
...
| G10 `IMPLEMENTATION_MISMATCH` — partially resolved (T18a, OSE) | Function-item acquisition and erasure remain incomplete (§7.7, §10; I18/M7). The earlier unchecked acceptance against an expected Function Type is repaired for single nongeneric receiverless references, including anonymous-Origin instantiation and fixed-Origin rejection; wider selection shapes are diagnosed as Unsupported. Unsafe acquisitions in the recorded T18a positions remain rejected. | Complete I18 selection against expected signatures, retained generic/Origin bindings and environments, Copy/erasure and receiver rules, and coverage of all acquisition positions. Passing Binding tests for the bounded comparison path does not establish function-item execution. |
