...
The current request (2026-09-22) continues compiler implementation from the unfinished Milestones M1–M15 and I/T items, respecting dependencies and specification boundaries. Milestone Programs (program numbers, P-task IDs) require explicit user instruction and are neither implementation targets nor stopping conditions; earlier checkpoint restrictions such as "stop at program N" are superseded. The named-argument boundary (2026-09-21 request) and finalized Origin redesign remain integrated. Draft edits and NativeAOT tests remain excluded.
...
Current position: **M11/I26 — T26ad–T26ah DONE (raw-pointer subplace projections, bounds-checked computed indices, in-place raw string comparisons, §21.1 C-layout formation diagnostics including written instantiations)**. This 60-minute continuation started at **2026-09-22 08:14:16 UTC** (17:14:16 JST), recorded before inspection; deadline **09:14:16 UTC**. Fresh inspection found HEAD `53ac0e4b` (documentation-only changes after the recorded `81926c3b` checkpoint) with a clean tree. Final tree: Debug/Release builds have zero warnings/errors, each full suite passes 11,212 tests, and 33 freshly generated pointer fixture sets pass 66 LLVM 22.1.8 O0/O2 executions. Stopped at **09:06 UTC** (~52 minutes): the remaining time could not hold another verified unit; I28 next steps are recorded below. General M11/M12 remain IN_PROGRESS. [Current evidence](PLAN_HISTORY.md#raw-pointer-subplace-continuation-20260922); [T26x–ac evidence](PLAN_HISTORY.md#raw-pointer-owned-continuation-20260922); [T26s–w evidence](PLAN_HISTORY.md#raw-pointer-continuation-20260922). Milestone Programs are neither targets nor stopping conditions. No draft edits or NativeAOT tests; historical checkpoint restrictions are superseded.

Previous focused work: **NAB — named argument boundary COMPLETE**. The formal specification, compiler and source migration are integrated. Debug/Release builds are warning-free; each passes 11,071 managed tests. Fifteen named-argument fixture sets pass 30 LLVM O0/O2 executions, and all 75 final Debug/Release fixture files match that tested snapshot. Release Milestone 11/18 regressions pass 55/53 checks. [Evidence and reproduction](PLAN_HISTORY.md#named-argument-boundary-20260922); [allocation measurements](Benchmark/NamedArguments.md). No remaining NAB action. Existing general compiler gaps remain below; NativeAOT was not run.
...
Program 18 remains complete (P18-G/O/V); its previous execution checkpoint and evidence are in [history](PLAN_HISTORY.md#program18-completion). Later Milestone Programs require explicit selection; unfinished M/I compiler implementation remains authorized. Their syntax is migrated where needed.
...
| Item | State | Acceptance / exact next action |
| --- | --- | --- |
| ORD1 | DONE | Finalized rules, examples and syntax summary are integrated into the formal owning chapters. Formal requirements are self-contained; the proposal is unchanged. |
| ORD2 | DONE | Contextual relation parsing, declaration attachment, closed/absent headers and set naming preserve source round trips and artifact reload. Existing lexer identifiers support the contextual keywords. |
| ORD3 | IMPLEMENTED — supported paths | Shared completion, relation proofs and variance-aware call inference connect schema/name collection, set projections, result quantification, omission, local/storage contracts and Loan obligations. Remaining general solver/specialization/callable dependencies are tracked in OSE3. |
| ORD4 | VERIFIED — supported paths | Library and affected milestone/test sources migrated. Debug/Release builds are warning-free and each passes 10,889 managed tests. LLVM 22.1.8 verifies and executes five Origin fixtures at O0/O2 (10 executions); Milestone 14/16 pass 48/57 checks in each compiler configuration. Bounded allocation checks pass. [Managed evidence](PLAN_HISTORY.md#origin-redesign-20260921); [native evidence](PLAN_HISTORY.md#origin-redesign-native-20260921). General Origin dependencies remain in OSE3. |

PN1–PN3 are complete; their scope and verification are preserved in [history](PLAN_HISTORY.md#parameter-names-implementation-20260920). General default/representation dependencies remain in I4/I5/I14/I18 and CR4.
...
| OSE3 | IN_PROGRESS — backlog | Extend the shared Origin solver to all specified mixed/cyclic principal and conditional proofs, inherit named-Origin/constrained specialization contracts, and complete I18 function-item/Callable and I10 custom-accessor generation. Preserve required-side rigid binders and actual Loans. Other OSE checkpoints are historical. |
...
Programs 1–18 have bounded completion. Programs 19–21 inform design only. General M/I obligations remain open as compiler backlog.
...
| M11 | IN_PROGRESS | T26a–i validate `#LibraryImport` declarations; T26j–o generate direct import calls (scalar, float and pointer signatures, quoted symbols), raw-pointer `null`/equality/conversions/displacement, and link root-project supplies (with T28g/h). T26s–w add bool and Copy/Owned tuple/array/struct pointee transfers, signed indexing, compound/increment updates and bound unsafe checks. T26x–ac add zero-stride diagnostics, Owned aggregate/string/enum Move/replacement, and concrete C layout. T26ad/ae add field/Tuple/fixed-array subplace projections of raw Places, with bounds-checked computed indices; T26af compares raw strings in place; T26ag diagnoses invalid C-layout declarations in Binding. Dependent pointee access, other non-consuming raw access of Non-Copy values, dependency-module supplies and provider identity remain. |
...
| I12 | M5 / R12 | IN_PROGRESS | Concrete/finite symbolic enum storage is extended by owned string tuple/enum decomposition, Move cleanup/results and exact nested string literal Patterns. P18 adds complete supported owned array/struct payloads with user destructors and zero-size cleanup anchors. Borrowed composite candidates/Subjects, general symbolic Patterns and full Pattern/regression closure remain. |
...
| I15 | M6 / R14 | IN_PROGRESS | Shared storage/selection/length bodies, concrete entries, fixed frames and bounded forwarding exist. G10a: shared bodies call ordinary concrete free functions (including Unit results) through their verified ABI, and bool/8–64-bit signed and unsigned integer leaves support checked `+ - * / %`, unary `- + not`, all comparisons and callback results. G10b/c add bool/integer Phi joins, owned string storage/output and concrete/generic forwarded string results. G10d–f add canonical Abort/require, Never calls/entries and bitwise/checked shifts. G10g/h add supported concrete constructor/receiver calls and checked 8–64-bit integer conversions. P18 verifies opaque composite arguments/results and exact per-Type destruction through the existing shared entries. General compound fields/calls, broader receiver layouts/defaults, borrowed strings, floating-point/128-bit/character operations and resource contracts remain unfinished. |
...
| I18 | M7 / R7, R14–R15 | IN_PROGRESS | Checked common calls, inline Copy-environment erasure and bounded concrete exclusive Callable witnesses execute. OSE checks expected signatures and complete Origins for single nongeneric receiverless function references; wider selection shapes reject. T18b makes the §7.7 unsafe rejection the complement of the direct-call callee position, so no value position stays unchecked, while the supported acquisition position list is unchanged. Next: complete overload/generic/member selection, function-item environment/Origin evidence and erasure, owner/Origin-polymorphic witnesses and indirect-call ABI. |
...
| I26 | M11 / R21, R24 | IN_PROGRESS | T26a: two plain nonempty NUL-free string arguments and a requirement (NativeRequirements or self-targeted NativeLibraries) of the defining module for the current target; reserved kernel32/kimi_backend exempt. T26b/T26e: bodyless `unsafe` group/receiverless struct type functions only, no own or enclosing generic/Origin parameters, specializations or default/optional arguments, and §21.5.2 reserved external names rejected, now read from the profile's own float-marker and provided-symbol catalog instead of a duplicated list. T26c: parameters/results limited to i8–i64, u8–u64, f32, f64 and `unsafe/T`, with Unit only as a result (`UnsupportedImportSignature_Kd`). T26d: one compilation-wide physical signature per external symbol (i8/u8 etc. and all `unsafe/T` share codes; `ConflictingImportSignature_Kd`). T26f: an import of one of the nine kernel32 declarations the generated runtimes emit (the seven of §22.5.6 plus the two test-runtime APIs, since one source is compiled in both builds) is accepted only through the reserved `kernel32` supply with that declaration's exact physical signature, and never for the `noreturn` `ExitProcess` declaration (`ConflictingRuntimeSymbol_Kd`); a recorded table is checked against both emitted runtime IR templates. Generated user/runtime definitions are all `__kimi_`-prefixed, which imports already cannot name. T26g: declarations of one external symbol must agree on the requirement `Kind` that selects dllimport, across source modules and including reserved `kernel32` (import) and `kimi_backend` (static) kinds (`ConflictingImportSupply_Kd`); an unresolved kind keeps only its own `MissingNativeRequirement_Kd`. T26h: a repeated `#LibraryImport` on one function is rejected as an invalid declaration, since one declaration selects one external symbol (§22.3.1); previously both attributes only stayed unresolved. T26i: an import of a reserved supply is limited to that supply's catalog — the reviewed project-owned kernel32 definition (§20.8.2.4) and the backend archive's provided symbols (§21.5.7) — and anything else is `UnavailableReservedImport_Kd`; since every backend symbol is a reserved external name, no `kimi_backend` import remains valid. T26j: a valid import attribute is validated without evaluating its operand and completes Binding (`Binding.LibraryImports` records function, library, symbol and Kind); `LlvmEmitter` registers one `FunctionAbi` per external symbol (Windows x64 C ABI, no extension attributes), `LlvmModuleWriter` writes `declare [dllimport]` unless the written runtime already declares the symbol, and ordinary call lowering uses it (5+ argument stack passing and `__imp_` indirection checked with llc). T26k: raw pointers are Copy `ptr` values (`ReferenceTypes.IsPointer`), `null` is a pointer constant, Binding admits same-Type pointer `==`/`!=` and rejects ordering/mixed Types, and control flow takes a resolved `null` argument's Type from Binding; pointer signatures declare `ptr`. T26l: `@` pointer/pointer and pointer/`usize` conversions (`ConversionBinding.Pointer`, `ptrtoint`/`inttoptr`, literal inputs fitted to `usize`, unsafe context except same-Type). T26m: symbols outside LLVM plain identifiers are written as quoted names with `\XX` byte escapes and recovered by the build-side admission. T26n: `p + n`/`p - n` with an `isize` count lower to `mul` by the signed stride and `getelementptr i8` without `inbounds`. T26o: compound `p += n`/`p -= n` on pointer locals. T26p: `*p` binds to the pointee Type and is writable. T26q/T26r: Copy reads and `*p = v` writes of non-bool scalar and pointer pointees lower to `load`/`store` with the pointee alignment. T26s: compound assignment through Copy scalar/pointer pointees preserves one address/read before the RHS and checked arithmetic. T26t: bool pointer loads/stores follow §21.1.4. T26u: scalar/pointer p[n] reads and writes use signed displacement; Binding-backed control flow checks unsafe permission for indexing and all four pointer displacement forms. T26v: prefix/postfix integer increments/decrements through pointer Places. T26w: whole Copy/Owned tuple/fixed-array/struct pointee reads/writes use verified aggregate transfers; Unit and zero-sized Copy values retain evaluation without byte access. T26y: whole Owned Non-Copy tuple/array/struct reads acquire a temporary owner and preserve Move/cleanup, including zero-sized destructors. T26x: zero-stride arithmetic/indexing is diagnosed by Binding for provably zero-sized Unit/tuple/array/struct Types, including substituted generic fields and direct bases; unknown layouts retain generation validation. T26z: Owned aggregate replacement destroys the old value at the raw address, then transfers the acquired source; StorePointer consumes source responsibility without creating a destination owner. T26aa: standalone string Move reads and replacement preserve complete handles and exactly-once release; non-consuming raw accesses are rejected rather than inventing an owner. T26ab: complete Owned enum pointees preserve tags, aligned strides and active-payload cleanup. T26ac: checked Layout metadata does not execute; concrete C structs use written order, ordinary LLVM structs and distinct cached physical shapes, with open/derived/empty/zero-sized-field forms rejected. T26ad: stored struct fields, Tuple elements and literal in-range fixed-array elements of `*p`/`p[n]` are writable raw Places in Binding (`ElementAccess.IsPointerPath`); ownership computes each level as a `PointerProject` value of canonical Type `unsafe/Part` (`Binding.PointerType`) and reuses the pointer load/store/replacement plans, so only that part is accessed and Non-Copy containers are not moved; lowering validates the layout offset and writes `getelementptr i8` without `inbounds`. T26ae: computed `isize` fixed-array indices below raw Places evaluate after the container address and are bounds-checked against the fixed length (`KIMI_E_INDEX_BOUNDS`) through the existing checked `ElementAddress`; static and computed levels share that opcode. Range/from-end indexing below raw Places keeps its existing handling (from-end fixed-array indexing is unsupported by Binding generally). T26af: string comparison operands that are raw Places (`*p`, `p[n]`, `(*h).label`) are inspected in place through their handle address (`OwnershipStringComparison` value = the pointer, Place -1); no owner, Move, Loan or cleanup is created, and lowering validates a dominating `unsafe/string` address. T26ag: Binding reports `InvalidCLayout_Kd` (§21.1) for `#Layout("C")` on open, derived or empty structs (static storage does not count) and for direct concrete zero-sized instance Fields (after storage preparation, using the zero-stride proof), independent of whether generation reaches the struct; T26ah: a written application of a generic C-layout struct (header or body Type, including under `unsafe/`) that makes a direct Field zero-sized, such as `P<()>`, reports `InvalidCLayout_Kd` at the application; header applications bound before storage preparation are deferred and checked once, and state resets per Bind. Inferred instantiations without written Type syntax still reject only at generation. Remaining: dependent (reference-containing) pointee access, other non-consuming raw access of Non-Copy values (for example comparisons of aggregates containing strings), C-layout diagnostics for inferred instantiations, C-exchangeability certification; dependency-module supplies (root-project supplies link since T28g), actual provider identity after supply resolution (§20.8.2.3), supply/symbol validation and complete C-exchangeability certification. |
| I27 | M12 / R22 | TODO | Implement package integrity/loading/packing/local publication/store lifecycle using fixed bytes. |
| I28 | M12 / R22, R24 | IN_PROGRESS | T28a supported source-module common generation and T28b Library inspection output are verified. T28c validates NativeRequirements and self-targeted NativeLibraries map records (Kind required after expansion, ContractId/Sha256 agreement, kimi_backend Sha256 assertion) at load and emission and diagnoses NativeBindings; T28e rejects repeated target/name entries that the dictionary formatter would silently overwrite. T28d: each `NativeLibraries` target reads the map shorthand or a `Name`/`Input`/`Package`/`ContractId`/`Sha256` record array (`NativeLibraryMap`/`NativeLibrarySupplies`, custom Tinyhand serializer writing the shorthand when every record is self-targeted); duplicate self-targeted names, repeated targets and repeated top-level `NativeRequirements`/`NativeLibraries` keys reject (as do repeated `CompileTimeSettings`/`Dependencies`/`TestDependencies` keys, which had the same silent-replacement defect); Package-targeted records reject `Kind`, the reserved `kernel32`/`kimi_backend` names and invalid identities, their inputs are path/`.lib`-checked even when unused, and after graph preparation must assert the resolved module's required ContractId and agree with its Sha256 (unrequired names stay unused). T28f: supplies of one module native name (per target), within a project and across the graph, merge only with agreeing ContractId/Sha256 assertions. T28g: publication writes `libraries` entries (Ordinal order among the reserved ones, manifest-relative inputs) for the root project's self-targeted supplies required by its imports; a required name without a supply, or an import defined in a dependency module, fails publication. `build` checks each such entry against the project's expanded Kind and Sha256 assertions using the actual input hash, and admits an object reference only as `__imp_S` for a `declare dllimport` or `S` for a plain `declare` of the verified IR. T28h: non-reserved inputs are linked from hash-named staged snapshots (§20.8.2.2). Remaining: combined-module native input records, actual archive member kinds/provider identity, content agreement of merged supplies and full requirement/supply connection validation at link time. |
...
Programs 12–17 retain their recorded target completion; P18 above records the current target. Program numbers are independent of M1–M15; earlier references to M1–M17 are historical numbering errors, not two missing milestones.

### Current next action

Programs 1–18 are complete; Milestone Programs are not implementation targets without explicit instruction. ORD4 is complete for its bounded paths; OSE3 remains backlog. The ordered compiler actions below are current.

1. I26/I28: T26s–ah implement the bounded raw-pointer operations, concrete C layouts and static subplace projections above. Next: I28 combined-module native records. `EmissionArtifacts.ResolveForeignSupplies` currently throws "Native supplies required by dependency modules are not linked yet." for any import whose function belongs to another module. Per §20.8.3/§18.5.2: (1) revise the link manifest to schema 4 with scoped requirement identities (defining module + logical name) on each `libraries` entry, preserved through content deduplication, and reject schema-3 records for multi-module inputs; (2) resolve each dependency module's imports against that module's own `NativeLibraries` supplies (T28d/T28f records); (3) extend `build` admission/hash checks to the scoped entries; (4) test with a two-module source graph whose dependency imports a self-supplied static `.lib`, including conflicting same-name supplies. Alternatively, I26 dependent (reference-containing) pointees, which need an explicit Origin/access plan rather than an invented Loan; raw-to-safe reference conversion stays unspecified (§5.6); never turn a read-only inspection into an owning Move or invent a safe Loan. Complete C formation diagnostics, C-exchangeable storage certification and remaining layout/reload/fragment interaction coverage; aggregate foreign argument passing stays outside the direct-import signature subset. Independent I28 work: combined-module native records connecting dependency-module imports to scoped supplies (§20.8.3, §18.5.2), then archive member Kind/provider identity and ambiguity validation (§20.8.2.3). Start each slice with a focused reproducer and verify Debug/Release plus fresh O0/O2 execution. I11 dynamic static initialization/shutdown and I6 general Origins/joins/backedges remain independent.
2. Close the rest of G10 with I18: give a named function group used where a Function Type is expected a real acquisition path that selects against the expected signature, covering overload groups, generic and member references. The unsafe half is done (T18b); `IsAcquisitionPosition` in `Kimi/Compiler/Binding/Binding.Expressions.cs` is the remaining bounded list, and broadening it requires the selection, Origin and erasure work above, not a larger position list. Earlier reproducers are in [the import-symbol evidence](PLAN_HISTORY.md#compiler-continuation-20260920-import-symbols); the position work is in [the T18b evidence](PLAN_HISTORY.md#compiler-continuation-20260920-unsafe-value-positions).
3. Complete independent product/test optimization and resource plans (I16/I30), reusing product entries without enlarging their frames or sharing classes.
4. After those prerequisites, extend T25/T26 to initialization/shutdown failures and complete product-plan isolation; retain the adopted interfaces and finite limits.
...
| G10 `IMPLEMENTATION_MISMATCH` — partially resolved (T18a, T18b, OSE) | Function-item acquisition and erasure remain incomplete (§7.7, §10; I18/M7). The earlier unchecked acceptance against an expected Function Type is repaired for single nongeneric receiverless references, including anonymous-Origin instantiation and fixed-Origin rejection; wider selection shapes are diagnosed as Unsupported. T18b closes the position gap for the unsafe rejection: every use that is not the callee of a direct call is rejected, so no value position remains unchecked. [Evidence](PLAN_HISTORY.md#compiler-continuation-20260920-unsafe-value-positions). | Complete I18 selection against expected signatures, retained generic/Origin bindings and environments, Copy/erasure and receiver rules, and coverage of all acquisition positions. Passing Binding tests for the bounded comparison path does not establish function-item execution. |
