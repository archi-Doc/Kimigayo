...
- **Verified implementation HEAD `48cd9152`:** session `bin/verify/20260926-032721-844-session-spec-implementation-review` passes warning-free Debug/Release builds, 12,576 tests each, 306 Shared native O0/O2 executions and the Release harnesses of Programs 1–23 and 29–32; all 3,613 native fixtures pass at O0/O2. No NativeAOT run.
...
- **Places, borrowing and iteration (2026-09-26):** the finalized proposal `draft/Design/2026-09-25 Places Borrowing and Iteration.md` is integrated into SPEC (`7787509d`) and implemented in four verified units `7dcd7a42`, `ebd1f301`, `b4f5d95a` and `38b8e9aa` (dereference, slot borrows and RHS-first updates; subject rule and reference-binding Patterns; `for` modes; element Places and implicit borrows at expected references). `ExclusiveSubject_Kd` is retired. The user's follow-up decisions (Subjects acquired as written, one shared reference through layers, Scalar result unification, reference-binding diagnostics, `LendingIterator`/`Iterator` split) and the reviewed implementation issues are integrated (`f6466cfb`) and implemented in per-unit commits through `de0a77db`: one adaptation table, one path-capability computation, member selection through layers, stored element Types, `ref/T` candidates of owned Scalar/Unit Subjects, and in-place string comparisons, string arguments and nested projections below shared elements. The following review rewrote SPEC.md and its chapters for consistency and plain English (`2abafa05`, `3d7e3cb5`) and fixed the reviewed implementation defects through `48cd9152`. The specification forms not yet parsed (`place ref/T` and `place uniq/T` results, Contract generic parameters, parameterized `associate` with formation Types, `T.(C).LentItem(a)`) and the user iteration/indexing protocols belong to P27/P28; the verified boundaries are in STATUS.
...
3. **Places and iteration syntax:** parse and bind `place ref/T` and `place uniq/T` results, Contract generic parameters, `associate Name(params)` with formation Types (`for Type`) and `T.(C).LentItem(a)`, then implement the user `Iterable`/`UniqIterable`/`IntoIterable` protocols and the `Indexable` family within P27/P28. Until then these forms stop at the parser (STATUS records the boundary).
