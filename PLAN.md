...
The compiler-wide plan covers unfinished implementation Milestones and Checklist items, respecting dependencies and finalized specification boundaries. The current focused request is step 4 of the adopted documentation Markdown migration: time/allocation benchmarks and measured improvements while retaining Markdig. Earlier timed compiler continuations are historical, not current authorization. NativeAOT and draft edits remain excluded.
...
Current focused checkpoint: **DM4 — benchmarks and measured improvements complete for the implemented parser API, plus a second tuning round**. Baseline `4c53331` for DM4; the second round (same day, baseline `cde175c`) fixes bare link destinations containing `<`, makes the parser a stack-only `ref struct` with stack scratch, shrinks retained nodes to 36 bytes, merges adjacent text, defers destination/title strings and removes the NUL-bearing search needle. Same-process comparison across six representative inputs reduces geometric-mean parse time by a further 33.1% and allocations by 33.7% over DM4 (0.395/0.170 of Markdig); details and limits are in the [benchmark report](Benchmark/DocumentationMarkdown.md#second-tuning-round-2026-09-20-after-dm4). Debug/Release solution builds are warning-free; all 10,187 managed tests, including 977 documentation tests, pass per configuration. The existing Markdig-backed `DocumentationMarkdown` API remains the product path. [Execution evidence](PLAN_HISTORY.md#documentation-markdown-tuning-20260920); general compiler items below remain backlog.
