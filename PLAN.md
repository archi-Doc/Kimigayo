...
The compiler-wide plan covers unfinished implementation Milestones and Checklist items. The current request (2026-09-21, timed continuation) authorizes work on unfinished implementation Milestones and Checklist items in dependency order; it supersedes checkpoint-specific restrictions such as "stop at program N" or "await the next target". Milestone Programs require an explicit user instruction and are not implementation targets. The finalized 2026-09-21 Origin redesign remains integrated with precedence over conflicting previous rules. Draft edits and NativeAOT tests remain excluded.
...
Current position: compiler-wide continuation from the [current next action](#current-next-action). T28d and T28f (I28) are verified; see the I28 row and [evidence](PLAN_HISTORY.md#native-supply-records-20260921). The ORD focus below is complete for its supported paths; its general dependencies remain in OSE3.
...
ORD1/ORD2 and the supported ORD3 paths are integrated; ORD4's managed and native verification checkpoint is complete. General Origin-dependent implementation continues under OSE3: extend the shared solver and specialization/callable contracts listed above, with focused acceptance, rejection and generation coverage. The bounded ORD4 regression does not complete those broader contracts. The following compiler-wide actions remain backlog.
