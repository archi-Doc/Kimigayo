...
The compiler-wide plan covers unfinished implementation Milestones and Checklist items, respecting dependencies and finalized specification boundaries. The current focused request is step 5 of the adopted documentation Markdown migration: switch the product facade to the independent parser and remove Markdig from compiler dependencies. Earlier timed compiler continuations are historical, not current authorization. NativeAOT and draft edits remain excluded.
...
Current focused checkpoint: **I26 foreign-import symbol agreement**. Baseline `7d2a8cb` (clean tree; the DM5 record below is its predecessor). T26f–T26i close the §21.5.2 runtime-declaration collision and supply-kind (`dllimport`) agreement parts of I26 in Binding, reject repeated import declarations and restrict reserved-supply symbols; T18a rejects unsafe functions acquired as values (§7.7). Warning-free Debug/Release builds and all 10,602 managed tests pass per configuration. [Execution evidence](PLAN_HISTORY.md#compiler-continuation-20260920-import-symbols). Provider identity after supply resolution, direct-call lowering, linking and native import execution remain open.

Preceding checkpoint: **DM5 — product migration complete**. Baseline `d871d45`. `DocumentationMarkdown` now uses the independent parser, immutable node/item APIs, iterative HTML output and structured URL resolution. Markdig remains only in comparison tests/benchmarks; compiler assembly/assets/manifests and fresh managed publish are free of it. Warning-free Debug/Release builds and all 10,561 managed tests (1,351 documentation cases) pass per configuration. [Product API](Kimi/Compiler/Documentation/README.md), [output measurements](Benchmark/DocumentationMarkdown.md#dm5-product-output-and-switch-2026-09-20), and [execution evidence](PLAN_HISTORY.md#documentation-markdown-product-switch-20260920). General compiler items below remain backlog.
...
| I18 | M7 / R7, R14–R15 | IN_PROGRESS | Checked common calls, inline Copy-environment erasure and a bounded concrete exclusive Callable witness execute. P14 adds direct per-call ref/T inputs and an exclusive borrowed-capture witness. General function-item erasure, owner/Origin-polymorphic witnesses and wider indirect-call ABI remain. T18a rejects unsafe functions acquired as values in initializer, assignment, argument, transferred-result, expression-body and array/tuple element positions (§7.7). G10 records the remaining unchecked acceptance: a safe function group still satisfies any declared Function Type without a signature check. |
...
2. Close the rest of G10 with I18: give a named function group used where a Function Type is expected a real acquisition path that selects against the expected signature, and extend the T18a unsafe rejection from its listed positions to every remaining value position. The reproducers are recorded in [the execution evidence](PLAN_HISTORY.md#compiler-continuation-20260920-import-symbols).
...
| G10 `IMPLEMENTATION_MISMATCH` — partially resolved (T18a) | A named function group used where a Function Type is declared is accepted without any check. `Binding` reports `IsComplete` with no issue for `func safe() -> i32 => 1` + `let g: (i32) -> i32 = safe` (wrong arity) and for `unsafe func raw() -> i32 => 1` + `let g: () -> i32 = raw` + `let v = g()`, which calls an unsafe function through an ordinary Function value with no unsafe context. `BindReference` marks a function group `Resolved` with a null Type for call selection, and `BindVariable` skips its `CheckTypeUse` when the initializer Type is null, so nothing rejects the use. §7.7 forbids taking an unsafe function as a value, assigning it, passing it or converting it to an ordinary Function Type (repeated for imports in §22.3.1); §10 requires a checked selection. R4/R15/R21/R27, I17/I18/M7 | Not fixed in this execution: the sound repair is the I18 function-item acquisition path (selection against the expected signature, Copy/erasure and receiver rules) plus the unsafe prohibition, and a narrow syntactic guard at one initializer site would leave assignment and argument positions accepting the same use. T18a now rejects an unsafe function acquired in a declaration initializer, an assignment source, a call argument, a transferred result, an expression body or an array/tuple literal element (`UnsafeFunctionValue_Kd`), including through a qualified member reference, while direct calls are unchanged. The remaining acceptance is unchecked: a safe function group still satisfies any declared Function Type without a signature check, and the value positions outside that list have no acquisition path at all. The sound completion is the I18 acquisition path; implement it with I18 and extend the rejection to every position then. |
