...
1. **Programs 34–39 sources:** author the remaining program sources per the README verification scopes (Program 39 per `draft/Design/2026-09-26 Semantics-Generic Follow.md`), record their first diagnostics in `milestones/stage-baselines.json`, and run the session verification.
2. **P39 decision:** review the memo; on acceptance integrate it into §3.4.1, §8.1.1, §8.9 and §13.5.5.1 and implement it before P28.
3. **Then P28, P31, P40, P26, P37** in the order of §4; the Property/object track follows. The §4.6.8 Slice cost evidence (G3) is collected with P28's iteration work.
