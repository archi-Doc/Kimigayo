...
The compiler-wide plan covers unfinished implementation Milestones and Checklist items, respecting dependencies and finalized specification boundaries. The current focused request is step 3 of the adopted documentation Markdown migration: conformance, differential and Kimigayo integration testing while retaining Markdig. Earlier timed compiler continuations are historical, not current authorization. NativeAOT and draft edits remain excluded.
...
Current focused checkpoint: **DM3 — conformance, differential and Kimigayo integration tests complete for the implemented parser API**. Baseline `2b97404`. Debug/Release solution builds are warning-free; 947 documentation tests and all 10,157 managed tests pass per configuration. The documentation fixture passes native O0/O2 execution from both configurations. The existing Markdig-backed `DocumentationMarkdown` API remains the product path. [Verification evidence and remaining boundaries](PLAN_HISTORY.md#documentation-markdown-tests-20260920); general compiler items below remain backlog.
