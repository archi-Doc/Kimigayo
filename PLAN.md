...
Current execution: **General compiler implementation — 30-minute implementation window (G10a)**, started 2026-09-19 04:54:48 UTC from clean HEAD `5d9e12c`. Scope limits from the user: no runs of existing tests, no example/milestone build checks, minimal new tests. G10a is DONE for its bounded criteria below; G10 as a family and M6/I15 remain IN_PROGRESS. [Evidence](PLAN_HISTORY.md#general-compiler-20260919-045448). The preceding KL execution is complete; its record and the T6n–T6r checkpoint remain in [history](PLAN_HISTORY.md#kimi-library-organization).
...
| M6 | IN_PROGRESS | Programs 8/9/11 shared storage, lengths, forwarding and selected Type specialization exist. G10a adds concrete direct calls and checked 8–64-bit integer scalar operations in shared bodies. Full universal proofs, operations and budgets remain. |
...
Finding G10 (M6/I15, pre-existing at HEAD `95731eb`; confirmed by rebuilding the HEAD compiler sources). Shared generic bodies admit direct calls only to generic targets (`GenericStoragePlan` Call handling), and a Unit-result call has no Place; any non-generic direct call therefore fails the shared validation. Example: a shared generic body whose parameter is `ref/T` and that calls `Console.writeLine` fails generation with "Shared operation refers to invalid storage or projection." (`func inspect<T>(value: ref/T)` / `    Console.writeLine("seen")` / `let v: i64 = 3` / `inspect(v)`). The same function with an empty body executes. This is an independent shared-generic lowering gap, not caused by T6n–T6p.

G10a (M6/I15; SPEC §21.2–21.3, §13.4 checked integer semantics) DONE for bounded criteria:
- Shared generic bodies call ordinary concrete free functions (no receiver, constructor, defaults, Type arguments or compiler-provided lowering) through a direct adapter bound to the function's verified ABI. Unit-result calls are admitted without a result Place.
- Shared scalar leaves cover `bool` and signed/unsigned 8–64-bit integers (previously `bool`, `i32`, `isize`).
- Shared scalar operations: checked `+ - * / %` with signed/unsigned overflow, division-by-zero and `MIN / -1` / `MIN % -1` Abort; unary `-` (checked), `+` and `not`; `< <= > >=` with the operand's signedness; `==`/`!=`, including on `bool`.
- Common-function callback results accept the same scalar set.
- Evidence: 14 new SharedConcreteCallTest cases pass in Debug and Release (warning-free builds). 14 fixtures pass 28 LLVM/native O0/O2 runs, and Debug/Release fixture bytes match. Existing tests were not run by user instruction, so regression evidence for pre-existing shared-generic fixtures is NOT_RUN. A diff review found no other consumer of the changed operator strings; `i32`/`isize` emit the same instructions as before.
- Remaining G10: the original example (`Console.writeLine` in a shared body) now fails earlier with "Generic storage requires finite owned fields, scalar values or Type parameters." because shared storage has no string leaves. `and`/`or` in a shared body fail `ValidateSharedGraph`, which rejects Phi values ("Shared result joins still use secured storage").

Next actions after G10a:
1. First, run the existing managed suites (Debug/Release) and the shared-generic native fixtures (GenericForwarding, GenericIteration, GenericStorage and the related families). This execution did not run them by instruction, so G10a is unverified against those tests.
2. Then continue G10 with Phi joins for shared `and`/`or`/conditional results, then string leaves (literal production, destruction policy, `Console.writeLine` lowering in the shared writer).
