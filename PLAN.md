...
- **Verified source HEAD** `a1f0ebd5`. Debug/Release builds are warning-free; each full suite passes 11,693 tests. Session evidence: `bin/verify/20260923-080549-session-p29-session6`; 202 Array native O0/O2 executions and all 23 harnesses (1–22 and 29, Release, 1,187 checks) pass. The closing CLI diagnostic audit leaves G17 open; P29 is not yet DONE.
...
- **Programs 1–22 and 29** have verified native execution. Programs 23–24 are authored targets with prior failed build probes; 25–28 and 30–38 are not written yet. Source creation or a successful target run alone does not complete an implementation milestone.
...
- **P29 progress (2026-09-23, sessions 2–6):** handles, literals, metadata, owned parameters/results, mutation, indexed replacement, Index overloads and owning iteration run natively. Session 6 adds checked capacity limits, allocation/transfer bounds and warm reuse (`fa3af716`, `0ac5dd7c`), Array slices/shared iteration (`f7e07b11`), control-flow result iteration (`e89dcc0a`), element borrows and concrete Non-Copy struct reads (`8300fbc5`, `d3ecd0ea`), the expanded harness and view regressions (`caad73ba`), and allocation-free warm Slice emission (`a1f0ebd5`). Program 29 is unchanged (SHA-256 `512684923B6A23722376FB8B845C565AB67A44ACD84D7DCDEE36AA8BA67246CE`) and passes 115 checks per configuration. Native probes verify no allocation for empty/capacity-preserving operations or shared views, linear growth-transfer bounds, shrink failure and capacity extremes; all five warm compilation stages allocate zero bytes. Remaining completion work: G17 diagnostic guards.
...
| 5 | P29 | Dynamic Array | IN_PROGRESS | §4.7 capacity, mutation, Non-Copy elements, owning iteration and mandatory allocation bounds are verified; the Array part of G3 is settled. Unchanged Program 29 and 115 harness checks pass in Debug/Release O0/O2. Shared element/view access and iteration are verified; G17 still needs ownership-stage Unsupported diagnostics before completion. |
...
1. **P29 diagnostic guards (G17):** reject unsupported zero-sized Array element layouts and shared `Array<string>` iteration during ownership analysis; strengthen the boundary tests and CLI harness to require `UnsupportedOwnership_Kd`, not merely an emission failure. Keep the formal requirements unchanged.
2. **P29 completion:** retain the immutable Program 29, its 115 passing checks and the native cost probes; after G17 passes focused tests, run session verification and earlier harnesses, then update STATUS/README and mark DONE only when all §5 conditions hold. General Slice/Index/Range and user iteration remain P27/P28.
3. **P30 next:** probe SPEC §13.4.1 Equatable/Comparable declarations, intrinsic witnesses and generic requirement calls; start with NaN-reflexive Contract equality versus built-in IEEE equality, then shared-borrow/Tuple composition. G11/G14/G16 and default/aggregate Copy-read work remain with their owning milestones.
...
| G17 | Closing audit at `a1f0ebd5`: `Array<()> = [()]` and bare iteration over `Array<string>` bind but fail lowering with `GenerationFailed_Kd` (no IR published), instead of an ownership-stage Unsupported diagnostic. Evidence: `bin/verify/20260923-080549-session-p29-session6/boundary-audit`. Add eligibility guards before these unimplemented paths and assert the exact diagnostic; the current boundary tests check only failure without partial IR. No new implementation unit was started after the 17:05:30 cutoff. | P29 |
