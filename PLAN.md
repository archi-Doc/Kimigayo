...
- **Verified source HEAD** `907513a3`. Debug/Release builds are warning-free; each full suite passes 11,356 tests. Session evidence: `bin/verify/20260922-152100-session-p22-instance-shapes`; all existing harnesses (1–12 and 14–19) and 168 generic native O0/O2 executions pass.
...
- **P22 progress:** each concrete call context of a verified generic body is re-analyzed under its substitution (`OwnershipAnalysis.AnalyzeInstance`) and lowered by the ordinary `BodyLowering` under the caller-facing entry ABI (`LlvmEmitter.LowerInstances`). Scalar functions, Never bodies, length-generic functions, generic struct constructors and field reads, borrowed-array element borrows/reads (scalar, aggregate and string elements), forwarded string references, concrete member-call receivers, callable values and borrowed Slice-handle fields (the library Slice iterator) now monomorphize. A refused-instance probe over the full suite leaves two shapes on the shared path: enum Pattern tests inside generic bodies (`present`/`unwrap`, 6 instances) and one Callable-constraint call (`apply`). Refused instances still use the shared path (`GenericStoragePlan*`, `LlvmModuleWriter.GenericStorage`/`SharedCalls`, `BodyLowering.Shared`); selected explicit specializations keep their selected body.
...
1. **P22 remaining instance shapes:** enum Pattern tests inside generic bodies (`Option<T>` matching in `present`/`unwrap`: `BodyLowering.Match` "Pattern test is not a pure whole-Subject inspection"), the Callable-constraint call (`apply`: "Invalid comparison Loan lifetime or conflict"), then general T-result forwarding (`forward<T>(value: T) -> T => identity<T>(value)`, still rejected by the transitional shared template). To find refusals, log `failure` in `LlvmEmitter.LowerInstances` temporarily and run the full Debug suite (do not commit the probe). P19's concrete Contract path is not completion of this migration.
