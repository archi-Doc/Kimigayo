...
Current position: **M11/I26 — T26x zero-stride diagnostics DONE; T26y Owned Non-Copy aggregate reads DONE; T26z replacement writes DONE; T26aa string pointees DONE; T26ab enum pointees DONE; T26ac C-layout investigation IN_PROGRESS; T26s–T26w retained complete**. This 60-minute continuation started at **2026-09-22 07:09:24 UTC**, recorded before inspection; deadline **08:09:24 UTC**. Fresh inspection found HEAD `77bd5038` and the previous pointer implementation, tests and documentation uncommitted; preserve these changes and verify affected paths against the current tree. T26x now diagnoses provably zero-stride arithmetic/indexing through substituted logical storage, including direct bases. Debug/Release each pass 11,159 tests and 32 fresh pointer-native O0/O2 executions pass. T26y adds Owned Non-Copy tuple/array/struct reads with existing temporary Move/cleanup; 161 focused tests pass per configuration and the extended read fixture passes O0/O2. T26z adds explicit StorePointer source consumption and in-place old-value destruction. Full Debug/Release suites each pass 11,168 tests; 38 fresh pointer O0/O2 executions pass. T26aa adds standalone string Move reads/replacement through the existing field-wise transfer/destruction path; direct non-consuming raw-pointee access remains rejected. Full Debug passes 11,175 tests, focused Debug/Release each pass 144, and eight final string O0/O2 executions (including lifetime audits) pass. T26ab adds complete Owned enum pointees with tag-preserving transfer and active-payload destruction. Full Debug passes 11,178 tests; focused Debug/Release each pass 117, and four final enum O0/O2 executions pass. Reused enum-case layout scratch removes the measured 80-byte-per-iteration temporary allocation. Next investigate the C-layout execution gate and implement a bounded supported layout slice if prerequisites permit. General M11/M12 remain IN_PROGRESS. [Previous verification and detailed history](PLAN_HISTORY.md#raw-pointer-continuation-20260922). Milestone Programs remain excluded as implementation targets and stopping conditions; no draft edits or NativeAOT tests. Earlier timed checkpoints impose no stop restriction on this request.
...
1. I26/I28: T26s–w complete the bounded pointer Place operations above. T26x completes provably zero-stride diagnostics. Next, extend Non-Copy Owned pointee Moves and replacement with explicit acquired-source consumption, old-value destruction and cleanup; never suppress an unrelated owner's destruction or invent a safe Loan. Dependent/reference-containing pointees and direct aggregate subplace access need their own Origin/access plans. Complete C layout before exposing C aggregate memory, and preserve rejection meanwhile. Independent I28 work: combined-module native records connecting dependency-module imports to scoped supplies (§20.8.3, §18.5.2), followed by archive member Kind/provider identity and ambiguity validation (§20.8.2.3). Begin each slice with a focused reproducer, then Debug/Release managed and freshly generated O0/O2 evidence. I11 dynamic static initialization/shutdown and I6 general Origins/joins/backedges remain independent.
