...
状態: 確定。ps1・プロンプトの実装基準は本書とする。runnerは未実装。

## QuickStart

「実行基盤を作成 → PLAN.mdを作成 → 実行」の順に進める。山括弧を対象に合わせて置き換える。

### 手順1: ps1・プロンプトの作成

```text
autoframe.mdを仕様の正本として、
...
### 手順2: PLAN.mdをプロンプトで作成

これはユーザーからの作成・更新依頼であり、自動実行中のWorkerとは区別する。[create-plan.md](autoframe/prompts/create-plan.md)は補助例で、本書だけでも作成できる。

```text
autoframe.mdの§2に従い、<プロジェクトのパス>/PLAN.mdを作成してください。
...
実行基盤の検証とPLAN.mdの確認後、プロジェクトルートで起動する。現時点ではrunner未実装のため、以下は実装後の使用例である。
