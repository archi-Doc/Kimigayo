...
| 31 | YES | FAIL (Debug/Release, O0/O2) | NOT_RUN | The Binding baseline stops at `entries[Key.init(1, 0)]` with `UnsupportedBinding_Kd`. Empty storage, capacity, search/mutation APIs, lookup Loans, churn reuse and reverse entry cleanup pass 302 related tests and 24 focused O0/O2 executions, including the unchanged target prefix (`bin/verify/20260924-061114-697-unit-dictionary-entry-verified`). Keyed assignment, iteration and remaining storage forms are pending; full-target status retains the [authoring evidence](../PLAN_HISTORY.md#programs31-33-authoring). |
