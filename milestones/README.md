...
| 31 | YES | FAIL (Debug/Release, O0/O2) | NOT_RUN | Dictionary declarations, typed construction and operation signatures now bind; the current Binding baseline stops at `entries.length` with `UnresolvedBinding_Kd`. Source-contract verification passes 580 related tests (`bin/verify/20260924-052813-991-unit-dictionary-source-contract-verified`). Runtime support remains pending; native build status retains the [authoring evidence](../PLAN_HISTORY.md#programs31-33-authoring). |
