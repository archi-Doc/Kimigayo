...
| 28 | YES | FAIL | FAIL (prior Debug/Release, O0/O2 probes) | NOT_RUN | TODO | `Kimi.Iterable` is declared in source, so `Drain<T>` and `Batch<T>` now bind; Binding first reports `UnsupportedBinding_Kd` at the generic Slice element borrow `self.values[index]@ref` in `Cursor.next`. User `for` protocols, Origin-related associated iterators and generic iterator-state ownership remain. [Authoring evidence](../PLAN_HISTORY.md#programs25-28-authoring). |
