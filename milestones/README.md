...
| 27 | YES | FAIL | FAIL (prior Debug/Release, O0/O2 probes) | NOT_RUN | TODO | Current Binding first reports `TypeMismatch_Kd` at `let middle: Slice<i32> = selection: do` (a `ResolvedRange` key); Range values, Index resolution and the Place foundation are verified, Index/ResolvedRange keys and the Slice operations remain. [Authoring evidence](../PLAN_HISTORY.md#programs25-28-authoring). |
