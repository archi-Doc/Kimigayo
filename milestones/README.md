...
| 31 | YES | FAIL (Debug/Release, O0/O2) | NOT_RUN | Dictionary declarations, metadata and operation signatures now bind; the current Binding baseline stops at `entries[Key.init(1, 0)]` with `UnsupportedBinding_Kd`. Empty handles, capacity management, transfer and cleanup pass 44 related tests and ten focused O0/O2 executions (`bin/verify/20260924-054545-189-unit-dictionary-storage-loans`). Entry operations and iteration remain pending; full-target status retains the [authoring evidence](../PLAN_HISTORY.md#programs31-33-authoring). |
