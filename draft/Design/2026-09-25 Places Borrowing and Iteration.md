...
直接保持する`obj/T`と`objuniq/T`は経路の権限内で排他アクセスを、`rc/T`・`arc/T`・`objref/T`は共有アクセスを提供する。`let`のobjハンドルもpayloadを更新できるが、共有経路越しにはできない。payloadはTakeを提供しない。

完全なpayloadを通常の`uniq/T`として借りることは、T全体の置換を許すことでもある。不完全なViewへの直接代入だけを禁止し、参照を別関数へ渡した置換を許す抜け道は作らない。

`@objref`／`@objuniq`によるobject Viewの借用・再借用、明示的なupcastは既存の形成・権限条件を保つ。これらはobject用の操作であり、`@ref`／`@uniq`のスロット借用とは区別する。完全payloadの値借用には`h@deref@ref`／`h@deref@uniq`を使う。借用や投影は参照カウントを増やさない。
...
通常の関数・関数値・Contract呼び出しで、次の引数適合を許す。Uは完全な直接の参照先型である。
...
期待型のない取得では元の完全型を使う。未知の型引数を決めるために、借用先・Scalar read後の型・都合のよいSemanticsを探索しない。未束縛のTそのものに渡す引数は通常取得からTを決め、その取得を束縛後に再解釈しない。宣言された`ref/T`／`uniq/T`の引数や、既に固定された期待型には前節の適合を使う。

```kimi
// n: ref/i32、r: uniq/Node。identity<T>(value: T) -> T。
let a = identity(n)        // T = ref/i32。
let b = identity<i32>(n)   // 固定されたi32へScalar read。
let c = identity(r)        // T = uniq/Node。通常取得なのでrをMove。
```
...
Takeの有無など型・経路から決まる条件は適用可能性で検査する。選択後の初期化状態・Loan違反では候補を選び直さない。`uniq/T`を`ref/T`の部分型とはしない。通常の結果型が既知なら、その利用位置のScalar readを認めるが、未知の戻り型を「参照を返すはず」と逆算しない。候補ごとの本体再解析や具体化後の選択変更も行わない。
...
後始末はローカル・一時値の生成とdefer登録の逆順、inline部分は宣言・要素順の逆順とする。独自destructorを自動的な部分破棄より先に呼ぶ。完全Move済みの値は破棄せず、独自destructorのない不完全集約は残る初期化済み部分だけを破棄する。内部のdestructorにも呼び出し直前のCompletenessが必要である。deferによる復元も実行順どおりに検査する。
...
`place(ref, T)`は共有借用とCopy可能な値の読み出しを、`place(uniq, T)`はさらに排他借用・置換を提供する。どちらもCompleteな実在Storageを公開し、Takeは提供しない。`place(owner, T)`や省略形`place(T)`は設けない。
...
外側Originの名前導入・量化・省略は、対応する通常の参照結果と同じとする。省略時は直接の借用入力の外側Originの共通範囲を使い、該当入力がなければ既存のstatic既定と本体検査に従う。receiverだけを優先しない。検索keyへ依存しないアクセサは`during self`と明記する。
...
結果モードだけが違うoverloadは共存できない。公開型Tは引数・明示型情報・期待されるPlace契約などから確定し、通常の期待値型から借用変換を逆算しない。型・モード確定後に、その利用位置の取得計画を検査する。`@ref`が外側にあるだけでPlace返却候補を優先しない。
...
両方のElementは同じ完全型であり、同じ添字・keyの選択を表す。読み取り・共有借用にはindex、更新・排他借用にはindexUniqを選び、その後で経路の権限を検査する。現在のreceiver型や取得の失敗から別の操作を試さない。引数式の内部や一般の関数結果を越えて更新要求を伝播しない。

receiver・keyは通常の順序で各一回評価する。標準の安全な配列は範囲外、Dictionaryはkey不在でAbortする。任意取得・挿入・削除は別APIとし、`tryGet`は`Option<ref/T>`などの通常値を返す。

Field・直接の固定配列投影・関数Place結果は共通の取得規則を使う。ただし直接の固定配列要素は§4.1のTake経路を保持でき、抽象的なIndexable適合はTakeを公開しない。Fieldの標準get／set権限やaccessor境界も維持する。
...
要求は定義元Contractの宣言Identityと束縛済み環境で識別する。同じ要求への複数経路は、関連型・Origin条件・実装対応が同値の場合だけまとめる。独立した同名要求は同じ型を返しても同一視しない。
...
実装は要求が許す全入力を受け入れ、要求以上の結果保証を与える。Originだけの違いで別の適合を選ばず、同じ対象・Contractに重複する実装を許さない。公開適合の型・条件・実装対応は有効アクセス範囲を満たす。完全型一般への適合ブロックやreceiver引数の並べ替えは追加せず、既存の名目型の適合宣言を使う。
...
guardの候補名は共有アクセスだけの候補Placeであり、まだarmのローカルではない。共有借用・Copy・Scalar read・比較を使えるが、Move、排他操作、代入、直接captureは不可。新しく作った候補依存のLoanはguard外へ持ち出せず、格納済み外部参照のCopyは既存依存を保つ。

Pattern検査からguardのcleanupまでCaseと候補場所を保護する。falseならcleanup後に次のarmへ進み、trueならcleanup後に元の場所から束縛する。副作用を巻き戻さず、guard中にpayloadをMoveしない。
...
型注釈だけでreceiver依存を除去しない。NoneにはItemが存在しないため、存在しないpayloadのLoanを保持しない。通常のCase・Loan解析に従う。
...
```kimi
for number in numbers       // ref/i32
    total += number         // 演算時のScalar read

for number in numbers@uniq  // uniq/i32
    number@deref += 1
...
Loanの**保護元**と**アクセス領域**を分離する。同じコレクションを保護していても、証明済みの非重複領域同士は重複と扱わない。
...
この保証は総称要求、ユーザー型への委譲、間接呼び出し、別コンパイルでも保持する。公開された検証情報に、結果と更新後の状態へ渡す領域・保護元を含める。呼び出し側でprivateな本体を再解析しない。
...
BorrowedはOriginスロットsourceを持ち、入力の外側借用Originに結び付ける。各結果型はI／C内部の依存も保つ。borrowedのItemのstepは実際のnext再借用に対応し、元の結果がreceiverから独立する場合だけその独立性を保つ。CursorアダプターはPlaceをOptionに入れず、通常の参照をpayloadにする。
...
| 領域 | 取り込み時に置き換える内容 |
| --- | --- |
| 型・取得（第3、10、13章） | 裸のNon-Copy取得をMoveへ変更。明示借用をスロットに固定。一般Copy readをScalar readへ限定。強制Moveを`@move`へ統一 |
| Field・配列・投影（第4、11、12、13章） | 全Placeの共通取得、Indexable、`@deref`、receiver省略、objectの完全payload条件。複合代入を右辺先行へ変更 |
| 関数・契約（第7–10章） | Place結果カテゴリ、完全型の関連型、Origin引数、要求の同定、適合と取得の順序 |
| Subject・Pattern（第14、15章） | 明示モード、一時Subjectの共有既定、Copy型も参照で束縛、Wildcardは取得しない |
| 寿命・cleanup（第15、16、17章） | 部分Moveの状態追跡、全早期終了、結果の連続保護、Loan分割。tryの通常取得も共通規則へ揃える |
| 標準列挙・生成（第4、22、21章ほか） | Iterator／Cursor／三つの列挙入口、標準Item、アダプター、参照ABI・性能保証 |
...
| 暗黙適合 | 所有値の共有借用、既存参照の再借用、固定Scalar read、無期待型での参照保持、overloadと明示Move |
| 投影 | 入れ子のref／uniq、objectのSealed証明、letハンドル、共有上限、単回評価、receiver経路の確定 |
...
| Place結果 | 借用元と内部Origin、複数return、Never、Unit fallthrough、一時receiver、cleanup、Callableと間接呼び出し |
| Pattern | 共有Copy要素の参照型、Scalar read、参照層、false guard、Wildcardと明示破棄の違い |
| Iterator | 所有・step・sourceの各Item、None、空・サイズゼロ、途中終了、借用保持中のnext、アダプターと総称委譲 |
| 領域分割 | 複数排他要素の保持、二重返却の拒否、key保護、Iterator破棄後の結果、元コレクションの更新禁止 |
| 表現・性能 | 参照相当のABI、追加確保・不要なCopyの不在、疎なDictionary、O0／O2で同じ意味 |
