...
| `tryFormat` | ローカルの`FixedBuffer`とアダプターを使い、`writeValue`で1回書式化する。成功時はアダプターの借用を終了し、`buffer@move.intoText()`で元配列に依存するビューを返す。書式化失敗時は`BufferFull`を返す。途中のバイトは領域に残り得るが、結果には含めない |

`tryFormat`では新しい空のバッファへ`Utf8Writer`だけで書くため、`intoText()`は再検証なしに成功する（§3.4）。`InvalidUtf8`を`BufferFull`へ変換するのではなく、失敗しないことを実装で保証する。ローカルの`buffer.text()`から返すビューは受け手借用に依存するため、そのまま関数外へ返さない。ゼロ長領域と空出力も許可し、長さ測定や自動再試行は設けない。
...
| `text()` | `ref/Self` | `Result<Utf8Slice, InvalidUtf8>`。§3.4の共有検証で確定済み全体のビューを返す |
| `validate()` | `uniq/Self` | `Result<(), InvalidUtf8>`。§3.4の排他検証で成功を記録し、反復検証を避ける |
| `clear()` | `uniq/Self` | 長さと検証済み長さを0に戻し、容量を保持する。消去は保証しない |
| `reserve(minimum)` | `uniq/Self` | §3.2のWindowを返す |
| `intoText()`（`FixedBuffer`のみ） | `owner/Self` | `Result<Utf8Slice{r}, InvalidUtf8>`、`origin r.source == self.source`。未検証の末尾だけを検証し、成功時は元領域の確定済み部分を共有借用して返す。確保・コピーなし。成功・失敗ともにバッファを消費し、元領域は解放しない |
...
受け手の取得は通常の規則に従う。直接の所有Placeでは、共有操作は`buffer.text()`、排他操作は`buffer@uniq.validate()`、消費操作は`buffer@move.intoText()`のように書く。排他参照経由のPlace（`self.inner.reserve(n)`）では印は要らない。

`bytes()`と`text()`の結果は受け手借用に依存する。`intoText()`はバッファの排他的な能力を消費し、元領域への共有Loanを結果に引き継ぐ。結果はローカルのバッファや共通状態を参照せず、元領域の寿命を超えては使えない。Originの等式だけでLoanを作ったり、既存の競合を解除したりしない。

共有Slice、Window、アダプターが保持するLoanと競合する検証・消費・書き換えは拒否する。検証状態は§3.4、破棄と再利用は§3.5に従う。一般の可変Sliceは導入しない。
...
`status(self: ref/Self) -> Result<(), BufferFull>`で失敗状態を確認できる。状態のリセット、rawなWindow、内部Writerへのアクセス、所有権を取り出す操作は公開しない。破棄と再利用は§3.5に従う。
...
`commit()`は共通状態の確定済み長さだけを更新し、ユーザーWriterへのコールバックを呼ばない。消費済みWindowは使用不可。未確定破棄は§3.5に従う。未初期化領域の読み取り、穴を空ける書き込み、任意長を確定する`advance(n)`は許さない。

`Utf8Writer`は公開`commit()`の代わりに、検証状態も更新する内部の確定操作（§3.4）を使う。Windowは常に共通状態から生じるため、ユーザーWriterが転送したWindowも同じ規則に従う。
...
検証済み長さは「先頭からここまでは有効なUTF-8」という記録である。共有操作は記録を変えず、更新には共通状態への排他権限を必要とする。

| 操作 | 検証済み長さの扱い |
| --- | --- |
| rawな`commit()` | 変更しない |
| `Utf8Writer`の内部確定 | 有効なUTF-8区間が検証済み部分に直結するときだけ、新しい確定済み長さへ進める。未検証部分を飛び越えない |
| `text()` | 未検証の末尾を検証するが、記録は変更しない |
| `validate()` | 未検証の末尾全体が有効なら確定済み長さへ進める。失敗時は記録・バイト・長さを変更しない |
| `intoText()` / `intoString()` | 消費した状態に同じ検証手順を適用し、成功時だけ完成したビュー／文字列を返す |
| `clear()` | 確定済み長さとともに0へ戻す |

検証済み長さと確定済み長さが等しければ、検証は0バイトで成功する。空または検証済みのバッファへ`Utf8Writer`だけで追記すると、この等式が維持される。

raw書き込み後に読み取りを繰り返す場合は、先に`validate()`で結果を記録できる。次は`Result`のエラー型が`InvalidUtf8`である関数内の例である。

```kimi
try buffer@uniq.validate()       // 未検証の末尾を一度だけ検証
let first = try buffer.text()    // 再検証なし
let second = try buffer.text()   // 再検証なし
```

`validate()`は`Utf8Writer`の失敗状態を変更しない。UTF-8の妥当性と書式化の成功・失敗は別の状態である。

### 3.5. 破棄と借用の終了

`FixedBuffer`・`WriteWindow`・`Utf8Writer`にはユーザー定義`deinit`を設けず、借用先を観測するフィールド破棄も持たせない。通常破棄は借用値の破棄だけで完了し、元領域・Writer・共通状態にアクセスしない。未確定Windowの破棄も、長さの更新・確定・出力・エラー通知を行わない。

このため、後続の使用や`defer`がなければ、借用は最終使用後に終了できる。管理値がスコープ内に残っていても、競合するLoanがすべて終了すれば元Writerを再利用できる。破棄時点を早めたり、一般の`deinit`規則を緩和したりするものではない。

`HeapBuffer`は別であり、所有する領域を通常破棄で1回解放する。`intoString()`の成功時は解放責任をstringへ移し、失敗時はバッファ側で解放する。`FixedBuffer`は消費・破棄のいずれでも元配列を解放しない。
...
`string`・`Utf8Slice`・ユーザー型に静的上限はなく、見積もりでは0として扱う。静的上限の加算はコンパイル時に行い、`MaxObjectSize`の超過は検出して見積もりを放棄する。Abortしない。
...
通常の伸長経路では、内部`HeapBuffer`への予約に、後続のリテラル長と有界値の上限の合計を先行確保分として加えてよい。現在の書き込み分は二重に数えず、無界の値の長さを得るために先行評価しない。

計算は次の順序とし、実際の必要量と見積もりを区別する。

1. 実必要長`required = length + minimum`を検査する。上限超過は§3.1の`KIMI_E_ALLOC_SIZE`とする。
2. 後続分の見積もり`hint`を求める。見積もり自体が表現できない、または`hint > MaxObjectSize - required`なら、この予約では加算を放棄して`required`を使う。
3. それ以外は`required + hint`を容量目標とし、§3.1の伸長規則を適用する。確定済み長さは増やさない。

**実必要長の超過はAbort、見積もりの超過は先行確保を放棄する。** 加算は検査後に行い、検査自体で整数オーバーフローを起こさない。先行確保を放棄しても式や書式化は再実行せず、必要に応じて後続の予約で伸長する。実際に試みた確保の失敗は既存のAbortとし、確保の再試行は追加しない。空出力や`minimum == 0`だけを理由に先行確保しない。

例えば上限をMとすると、長さM−5の文字列と数値`0`の出力は収まる。数値の最大長20を加えると超過するが、その見積もりだけを放棄すれば書き込みを続けられる。

この加算は補間・`Text.toString`の内部`HeapBuffer`に限る。`$tryWrite`やユーザーWriterには適用せず、予約引数と呼び出し回数を維持する。見積もりを放棄する場合の確保回数は§6.3に従う。
...
`HeapBuffer`本体はコンパイラだけが保持し、ユーザーコードには借用アダプターだけが渡る。§3.4の検証済み状態が維持されるため、`intoString`は再検証なしに成功し、生成コードはその失敗分岐を省略できる。
...
| 検証済み長さ == 確定済み長さの標準Writerの`text()`・`validate()`・`intoText()`・`intoString()` | 追加検証0バイト。ビュー取得・消費操作は追加確保・コピーなし |
| raw書き込み後に`validate()`が成功し、追記・変更しない標準Writer | 後続の`text()`で追加検証0バイト |
| 有界な補間の所有結果 | 書式化処理のヒープ確保1回以内、完成時コピーなし |
| 無界の値が`string`または`Utf8Slice` 1個だけで、他は有界な補間の所有結果 | 静的見積もりとその値の予約時の先行確保分が上限内なら、ヒープ確保2回以内。§4.4の加算で実長と後続分を一度に予約する。見積もりを放棄する場合は、この回数上限を適用せず通常の伸長に従う |
...
- 整数・浮動小数点の境界と表記、UTF-8検証、NUL、空出力、§4.4の最大長。
- コンパイル時の見積もりと実行時の先行確保分の上限超過を区別すること。実必要長が収まる場合は見積もりだけを放棄し、実必要長の超過は`KIMI_E_ALLOC_SIZE`になること。伸長計算の境界、空予約、見積もり放棄後の確保回数も確認する。
- スタック領域上限を超える有界な補間がヒープ経路になること。
...
- 検証済みビューとの競合書き換え・`validate()`・消費を拒否し、確定したUTF-8に不完全な文字を含めないこと。
- raw書き込み後の`text()`が記録を更新せず、`validate()`は成功時だけ検証済み長さを進めること。失敗時の状態不変、追記後の末尾だけの検証、`clear()`後の再利用、書式化の失敗状態をリセットしないこと。

#### 7.2.2. 寿命と破棄

- `intoText()`と`tryFormat`の結果が元配列に依存し、ローカルの管理状態には依存しないこと。成功・失敗ともに`FixedBuffer`を消費し、元配列を解放しないこと。ビュー生存中の競合書き換えと、元配列より長い寿命への脱出を拒否すること。
- 借用管理型の通常破棄が借用先を観測せず、最終使用後に元Writerを再利用できること。後続の使用や`defer`がある場合はLoanを保持すること。ユーザー定義`deinit`を持つ型には既存の破棄時検査を適用すること。
- Originの明示形と省略形の適合が一致し、返却ビューの依存先とLoanを保持すること。
...
- 直接の所有Placeを`Text.fixed`・`Text.writer`・`Text.tryFormat`・`$tryWrite`の排他位置へ裸で渡すとエラーとなり`@uniq`を提示すること。`validate()`の排他受け手も同じ規則に従い、借用値の`writer`は裸で再借用できること。`commit`・`limit`・`intoText`・`intoString`の裸の呼び出しがエラーとなり`@move`を提示すること。埋め込み式が元のPlaceを転送しないこと。
...
| §22.1・§22.1.1、SPEC.mdの宣言索引 | `Stringify`とstringの「Stringify」操作を削除し、公開Contract・型・`Kimi.Text`と本案の関数を登録する。`validate()`、`intoText()`の契約、検証状態と借用管理型の破棄条件を含む |
