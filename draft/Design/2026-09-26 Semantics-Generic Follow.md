...
層をたどる既存の位置では、ペア層も一段進んで `U` に移り、その位置の既存規則を繰り返す。停止するのは、必要な宣言・構造・型が見つかった層か、形状不明の型である。形状不明の型では公開制約で使える能力だけを使い、具体化後に探索を再開しない。例えば `s/(ref/V)` のメンバー選択は、ペア層の後も `ref/V` から `V` へ進む。

| 位置 | 規則 |
| --- | --- |
| メンバー・添字・receiver の選択（§3.4.1） | ペア層は宣言を持たないので `U` へ進む |
...
inspect(c[i])              // X = s/T。推論ではペア層をたどらない。
...
単相化では、`owner` はアドレスをそのまま使い、`ref`／`uniq` はポインタを一回読み出す。実行時の判定も割り当ても発生しない。将来の総称コード共有では、`s` を共有キーに含めるか、追従を Type policy の操作にする必要がある（§21.3.3 の設計に委ねる）。
