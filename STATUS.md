...
The program 12 execution verified the recorded working-source state with **8,531 managed tests per Debug/Release configuration**, **117 relevant standalone fixtures / 234 native O0/O2 executions**, and **1,058 checks for programs 1–12**. The [verification audit](bin/milestone12-work-20260918/verification.json) binds these results to source/compiler/test identities and checked program reports. Historical larger native-family counts below describe their own runs and were not repeated wholesale. See [§7](#7-verification-records).
...
`KimiLibrary` currently has **22 catalog entries, 10 validated**: Copy, Owned, Callable, writeLine, Option, Result, Sealed, replace, exchange and swap. Twelve catalog slots remain missing. Catalog shape/identity validation is distinct from runtime implementation, and the complete required library is not available. `CoreCatalogTest` confirms these counts; the old six-of-eighteen description is superseded. The stale object-spelling deferral comment in KimiLibrary does not supersede the specified object APIs (PLAN G4).
...
Ownership and native lowering support whole-value updates on complete mutable owner locals (scalars, strings and supported aggregates) and concrete scalar/struct/fixed-array uniq targets whose contents are proven Owned. Target Loans begin before later arguments; replace retains original-location destruction, exchange/swap transfer old-value responsibility without user destruction. Aggregate exchange results no longer use the function result slot as an input-Origin anchor. Borrowed lowering's Owned condition is an **implementation limit**, not an added API constraint.
...
| Defaults support scalar operations/conversions/selections/do, contained transfers, scalar/Unit locals and updates/finite loops, require and Copy scalar subplace reads from preceding owned arguments. Definite non-Copy prepared acquisition reports `DefaultArgumentMove_Kd` at declaration time. | Prepared parameters cannot be mutated or moved; owned referent/getter/borrow/capture paths require further plans. Declaration diagnostics do not certify unsupported runtime forms. |
...
`LlvmEmitter` supports the windows-x64-v1 Application profile, implicit top-level entry or eligible `public func main() -> ()`, with final Binding/startup/ownership required. Supported nested groups/structs/enums/Contracts and bounded generic/closure/default paths are admitted. External-module common generation, Library output and unsupported bodies remain rejected, including unused bodies. Its broad diagnostic wording about declaration containers does not mean all nested containers are rejected; `SupportedContainers` defines the admitted subset.
...
| Enums/Patterns | Concrete i32 tag/aligned payload, active-case transfer/destruction; finite symbolic enum construction/results; scalar/string/char/Unit matches and guards; nested tuple/enum scalar/Unit guard candidates with separate body acquisition | Composite match generation requires destruction-free layouts; owned destruction and borrowed candidate/Subject paths are incomplete. A valid enum constructor does not prove every Pattern form executable. |
| Shared generics | Universally checked supported storage CFGs, conditional stored-field CopyOrMove, length-dependent array transfer/destruction, logical lengths, finite symbolic enums, exact fixed scratch, concrete ABI entries and deduplicated policies | General universal operations/effects, symbolic compound fields, arbitrary borrowed generic ABI, owned-result forwarding and all optimization/resource contracts remain unfinished. No per-instantiation source rebinding. |
| Specialization/forwarding | Closed Type specializations checked against original contracts, selected independently of overload choice; shared direct-call adapters, indexed shared borrows without Copy on T and checked shared i32 arithmetic | Length/receiver/constrained/defaulted headers and explicit specialization Origin binders unsupported. Concrete context expansion is bounded at depth 128; not full resource-contract coverage. |
| Common functions/erasure | Checked indirect calls and shared receiver protection, scalar/Unit snapshots in an 8-byte inline environment, owned common-handle Move/return/cleanup. An acquired Copy concrete Shared environment can erase inline without consuming the original Copy value | Explicit parameter Types and supported fixed signatures required. Existing common-call result support includes scalar/Unit/Never; the new concrete path has a narrower result ABI. Existing-value erasure is bounded to Owned Copy environments of at most 8 bytes and supported scalar/Unit result ABI. Owned Non-Copy/heap erasure and general function-item erasure remain unfinished. |
| Concrete closures | Distinct environment identity, scalar/Unit/string and supported nested concrete captures, mutable snapshots, explicit nested returned closures, direct Shared/Exclusive/Consuming calls, borrowed exclusive receivers, reverse remaining-capture cleanup and empty environments | Expression results infer; block bodies require a result annotation. General aggregate/borrow captures, implicit capture propagation across missing enclosing environments, generic capture effects, partial environment moves and full Origin-dependent signatures remain incomplete. A wider source form can bind yet still be rejected by ownership or generation. |
| Callable constraints | Scalar `Callable<uniq, S>` calls through `uniq/F`, including repeated mutation, use a concrete compile-time entry adapter within the shared generic body. No erased common handle or per-instantiation source rebinding is introduced | Binding retains literal ref/uniq/owner contracts; executable witness coverage is narrower. Owner acquisition witnesses, general overloaded/Origin-polymorphic signatures and arbitrary callable parameters/results remain unfinished. Shared callback adapters now admit i32 as well as bool/isize/Unit results. |
...
| Sequences | Two-word full Slice/range handles, backing Origins/Copy, scalar Copy reads and indices snapshots; built-in ResolvedRange iteration, shared range iteration and supported scalar/Copy aggregate fixed-array iteration | Full Index/from-end/bounded Range/Slice APIs, direct Slice/collection/consuming iteration, tuple iteration bindings, general user Iterable/Iterator dispatch and arbitrary Slice element results remain incomplete. Mixed Slice provenance widens conservatively. |
| Collections/objects/FFI | Collection/capture/pointer syntax, declaration constraints and runtime `is` Binding have foundations | Dynamic Array/Dictionary mutation and cost contracts, object/rc/arc/Weak allocation/lifecycle/refinement and complete C/pointer/import execution are not established. Runtime Contract Views remain deferred. |
...
Package loading/pack/publish/store commands, portable content/semantic records, cross-module common native generation and Library output remain incomplete. Source serialization reparses/rebinds; it is not persisted authoritative semantic proof. Native manifests distinguish semantic validity from connection/input integrity.

## 6. Unconnected Product Features and Performance Infrastructure

| Area | Implemented scope and remaining limits |
| --- | --- |
| Language tests | Product mode excludes TestSources without reading them, skips test dependency resolution and removes inline #Test bodies from ordinary lookup/startup/analysis/generation while retaining selected syntax/target diagnostics. Test discovery/all-body checking, `$expect`/`$require` and the bounded runner remain unfinished. Specified false `$require` Aborts after condition/message temporary cleanup; it is not helper return. Public profile details remain PLAN G2. |
...
| Performance and NativeAOT | Saved workload-specific allocation results only; no new measurement. NativeAOT NOT_RUN | No universal allocation/speed claim; NativeAOT was neither run nor made a requirement. |
