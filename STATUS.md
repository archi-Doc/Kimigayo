...
The **2026-09-22 foreign-call continuation** lets a valid `#LibraryImport`
declaration complete Binding and generates direct calls to it inside `unsafe`
blocks for integer, `f32`/`f64` and raw-pointer signatures: one declaration per
external symbol, `dllimport` for `import` supplies, sharing the runtime's own kernel32
...
Stored struct fields, Tuple elements and fixed-array elements of `*p` and `p[n]`
(`(*p).field`, `(*p).1.0`, `p[n][i]`) are raw Places: reads, writes, compound updates and
increments displace the verified address by the stored offset (Kimigayo or C layout) and
access only that part, including Copy/string fields of Non-Copy containers. Computed `isize`
array indices there are bounds-checked against the fixed length. String comparisons read raw
string Places in place without a Move or release. Other non-consuming raw access of Non-Copy
values and dependent pointees remain unsupported. Concrete C struct layouts now preserve written order
using ordinary LLVM structs, including tested nested and instantiated generic fields and
Non-Copy destruction. Explicit Kimigayo layout uses the default alignment-sorted order.
Binding diagnoses `#Layout("C")` on open/derived/empty structs and on direct concrete zero-sized fields (`InvalidCLayout_Kd`); written generic applications such as `P<()>` that make a field zero-sized are diagnosed at the application; inferred instantiations reject at generation; broader C formation
diagnostics and exchangeability certification remain incomplete. Binding rejects arithmetic
and indexing with provably zero-stride pointees, including
empty aggregates, substituted generic fields and inherited empty storage. `build` links the
project's own self-targeted supplies required by its imports (manifest `libraries` entries in Ordinal order,
`Sha256` assertions checked against the linked bytes, object references admitted only
for declared foreign symbols). Kernel32 imports (including `VirtualAlloc`/`VirtualFree`
with pointers) and a Clang-built static supply with six mixed scalar arguments execute
at O0/O2. Symbols outside LLVM plain identifiers, such as MSVC-mangled names, are
...
nested composites. Zero-sized arrays/Tuples that require cleanup
retain a minimal aligned address anchor while keeping their logical size/stride
zero and destroying every logical element. Existing generic shared bodies and
concrete entries preserve conditional Copy/Move and per-Type destruction through
Box, selection, relay, deferred result delivery and enum packaging.

The new cases check selected/unselected and unused payload cleanup, Copy source
reuse, partial Pattern bindings, zero-sized/empty values, early return, Abort,
distinct destructors at equal sizes, rejected invalid ownership/corrupt plans,
and serialized reload. Warning-free Debug/Release solution builds each pass
**10,993 managed tests**, including 32 new cases. The target harness passes 53
checks per configuration; related native regression passes 156 fixture sets /
312 O0/O2 executions, with all 780 fixture files identical across configurations.
Programs 1–17 pass 102 Release regression checks. The
[final identity audit](bin/milestone18-work/verification.json) and
[execution record](PLAN_HISTORY.md#program18-completion) retain the failures,
fixes and exact verification scope. This does not add
borrowed aggregate guard candidates, general symbolic Pattern matching or later
Contract/inference/specialization features. NativeAOT is excluded by instruction.
...
projections and declaration-attached `==` / `outlives` relations replace the old
function lists and aggregate Origin applications. Existing omission defaults are
preserved after explicit relation completion. The finalized proposal is unchanged.

Binding retains scoped relations, fixed and quantified dependencies, and shared
variance-aware inference/proof operations for calls and callable contracts. Local
initializers, complete annotations without initialization, nested signature
boundaries, fragment agreement and artifact/source round trips have focused tests.
Phantom slots remain invariant without structural evidence; verified Slice metadata
is retained. Owned checks preserve unused generic arguments, raw-pointer pointees
and fixed callable dependencies. Origin equality does not merge actual Loans.
Supported lowering and LLVM IR output accept implicit named Origins without adding
runtime arguments. Standard library and affected milestone/test sources are migrated.

Debug and Release solution builds have zero warnings/errors; each passes **10,889
managed tests**, with no failures or skips. On the new measured equality-call
workload, warm Binding allocates at most 288 bytes per pass; ownership analysis and
IR writing allocate zero. These are bounded checks, not universal performance claims.

This is not full compiler conformance. General mixed/cyclic principal inference,
...
The 2026-09-20 Origin syntax and elision checkpoint is superseded by the redesign
above. Its schema/storage substitution fixes, artifact and allocation evidence
remain recorded in [the integration history](PLAN_HISTORY.md#origin-syntax-elision-20260920)
and [the surrounding-code review](PLAN_HISTORY.md#origin-review-20260920). Its
function Origin lists and aggregate Origin applications are no longer accepted.
...
build probes; programs 15–18 are now superseded by the completions above. Programs
19–21 retain their recorded limitations; 22–38 remain uncreated. The restructuring
...
| --- | --- | --- |
| Program 18 | PASS: warning-free Debug/Release builds, 10,993 managed tests each, 53 target/variant/rejection checks each, 156 fixture sets / 312 native O0/O2 executions, 102 program 1–17 regression checks | [Final identity audit](bin/milestone18-work/verification.json), [history and commands](PLAN_HISTORY.md#program18-completion). Exact output/exit/destruction, zero-size anchors, pinned LLVM 22.1.8 and byte-identical fixture reuse. NativeAOT NOT_RUN. |
