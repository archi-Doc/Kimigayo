...
| Core.writeLine and executable startup | Implemented source forms | Partial; canonical function and startup selection, C.23 | Partial; startup contracts, whole-Place ownership and cleanup, C.23–C.24; literal-output gate, C.34 | Partial; one implicit literal call, C.34 | Partial; native literal-output Application, C.34 |
...
The existing native helper archive is adopted as package 1.0.0 with a reproducible actual SHA-256; [the adoption record](backend/windows-x64/ADOPTION.md) distinguishes it from arbitrary local rebuilds. The provided LLVM directory reports 22.1.5, so the configuration accepts its path as a hint while manual profile validation rejects that version. Formal verification uses LLVM 22.1.8; the supplied kernel32 file matches SDK 10.0.22621.0 x64. See [the example and commands](examples/Hello/README.md) and [implementation details](doc/Changes/2026-09-13%20Minimal%20Executable.md).
