...
- [15. Ownership and lifetime analysis](spec/15-ownership-and-lifetime-analysis.md)
  ...
  - [Subject rule](spec/15-ownership-and-lifetime-analysis.md#1516-match-acquisition-and-lifetime): a `match` or `for` Subject is acquired like any expression, except that a bare Place is borrowed; the mode is Shared for shared borrows and `ref` values, Exclusive for `@uniq` and `uniq` values, and ByValue for owned values such as `@move` results and temporaries; shared and exclusive paths bind `ref/T` and `uniq/T`, owned Subjects transfer their parts, and guards see `ref/T` candidates.
