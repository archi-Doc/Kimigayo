...
- [15. Ownership and lifetime analysis](spec/15-ownership-and-lifetime-analysis.md)
  - [Lending rule](spec/15-ownership-and-lifetime-analysis.md#1515-movable-places): a bare Place Copies only a proven-Copy Type and never Moves; `@move` is the only transfer; a new exclusive lending of an owned Place needs `@uniq`/`@objuniq` except for a [Receiver Expression](spec/07-functions-and-callable-values.md#73-explicit-receivers), which is acquired implicitly; `@ref`/`@uniq` borrow the written slot, `@deref` selects the referent, and at fixed expected Types borrow values are Reborrowed and nested reference layers yield one shared reference under the [common adaptation](spec/10-overload-resolution-and-inference.md#102-common-adaptation-at-expected-types).
  - [Subject rule](spec/15-ownership-and-lifetime-analysis.md#1516-match-acquisition-and-lifetime): a `match` or `for` Subject is acquired like any expression, except that a bare Place is borrowed; the mode is Shared for shared borrows and `ref` values, Exclusive for `@uniq` and `uniq` values, and ByValue for owned non-reference values such as `@move` results and temporaries, while a transferred or returned reference keeps its own mode; shared and exclusive paths bind `ref/T` and `uniq/T`, owned Subjects transfer their parts, and guards see `ref/T` candidates.
