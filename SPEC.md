...
Object compatibility uses two distinct terms: [ObjectCallCompatible](spec/12-expressions.md#12441-public-status-and-use) describes call operations compatible with object-borrow or base-subobject-projection calls; [ObjectViewCompatible](spec/08-generics-constraints-and-contracts.md#85-runtime-contracts) describes Contracts eligible as object View Targets, such as in `obj/C`, `objref/C`, and `objuniq/C`. These names replace the former call-compatibility and runtime-view-usability terminology without changing their rules or adding an Attribute. Contract-target object Views remain within the deferred runtime Contract extension.
