...
C.52 supports bool, the ten integer Types through 64 bits, Unit and owned string Subjects; literal, wildcard, whole-Subject binding and grouped patterns; and existing scalar/string results, functions, loops and defer. Every match must be exhaustive. Guards, enum/Tuple decomposition, borrowed Subjects, char and floating-point execution remain unsupported. A covered arm still receives diagnostics. See [STATUS C.52](../../STATUS.md#c52-unguarded-whole-subject-match-execution-2026-09-13) and [backend setup](../../backend/windows-x64/README.md).
