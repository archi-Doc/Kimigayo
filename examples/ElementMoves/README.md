...
The implementation tracks referenced paths rather than expanding all declared array elements. Copy acquisition remains Copy. Dynamic Copy reads and replacements require a complete known prefix; dynamic Non-Copy extraction remains unsupported. Initial construction of an uninitialized array one element at a time remains forbidden. Direct inspection of Non-Copy elements by comparison or shared arguments is also rejected until element-borrow plans are implemented; these uses cannot silently Move the element. Parameter/temporary/borrowed receivers, struct/Property paths, user `deinit`, retained borrow dependencies and dynamic collections are outside this implementation unit.
