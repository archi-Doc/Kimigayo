...
編集範囲の検査は生成状態を除いたGit追跡対象・未追跡非ignoreファイルを対象とする事後検査であり、OSの書込制限ではありません。doc/は仕様調査から除外しますが、無断変更を検出するためrunnerのhash対象には残し、Implementationでもautomation/、doc/、AGENTS.md、.codex/、.agents/の変更を拒否します。ignoreされたファイル、Gitのindexや履歴、外部サービスの変更を防止する仕組みではありません。
...
正式な仕様の根拠はSPEC.md本文です。doc/は正式な仕様ではないため検索・読込・参照・仕様統合・実装/監査の根拠から除外し、SPECや過去の引継ぎに採用済み・優先と書かれていても例外にしません。本文にない詳細はdoc/から補完せず、不足と影響を記録して独立作業を進めます。既存計画のdoc/由来の要件・本文統合タスクは次のPlanで本文と照合し、対象・依存・指摘を見直します。NativeAOTは明示指定時だけ実施し、未決定の言語仕様を自動承認しません。

`Invalid plan task row: | [リンク名](...) ...` は、通常のMarkdownリンクをタスク行と誤認していた旧runnerの不具合です。現在はリンク行を除外し、タスク行の形式・ID重複・未完了必須項目の検査を維持します。修正後は`-Resume`で部分変更を保持したままPlanから再確認できます。
