...
- タイムアウトでは子プロセスツリーを終了します。Ctrl+C等の中断後も途中の作業を保存したまま、明示的に再開してください。

## 記録と再開

- IMPLEMENTATION_PLAN.md: 一意なID、依存、完了条件、検証方法を持つ実装計画。
- AUDIT_FINDINGS.md: AF-0001形式の指摘台帳。Planが初回作成し、各段階が採否・対応・検証を引き継ぎます。
- STATUS.md: 現在の対象ID、差分、検証、残作業、次の操作、外部待ち条件。
...
編集範囲の検査はdoc/と生成状態を除いたGit追跡対象・未追跡非ignoreファイルを対象とする事後検査であり、OSの書込制限ではありません。
...
- 旧版の状態や破損状態は自動変換せず拒否します。状態を確認し、必要ならResumeなしで開始してください。
...
プロンプトはdoc/除外とNativeAOTの明示指定条件を共有します。このループだけで、未決定の言語仕様を自動承認することはありません。
