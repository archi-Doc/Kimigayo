...
public class ScalarDefaultEmissionTest
{
    ...
    [Theory]
    [InlineData("Literal", "func f(x?: () = ()) => writeLine(\"ok\")\nf()")]
    [InlineData("Chain", "func f(x?: () = (), y?: () = x) => writeLine(\"ok\")\nf()")]
    [InlineData("Select", "func f(x: bool, y?: () = (if x => () else => ())) => writeLine(\"ok\")\nf(true)")]
    [InlineData("IfNoElse", "func f(x: bool, y?: () = (if x => ())) => writeLine(\"ok\")\nf(false)")]
    [InlineData("Do", "func f(x?: () = (scope: do => exit to scope)) => writeLine(\"ok\")\nf()")]
    [InlineData("Loop", "func f(x?: () = (loop => exit)) => writeLine(\"ok\")\nf()")]
    [InlineData("Local", "func f(x?: () = (scope: do\n    let n = ()\n    exit to scope: n\n)) => writeLine(\"ok\")\nf()")]
    [InlineData("BeforeScalar", "func f(x?: () = (), y?: i32 = 3) -> i32 => y\nif f() == 3 => writeLine(\"ok\")")]
    [InlineData("Supplied", "func f(x?: () = (loop => continue)) => writeLine(\"ok\")\nf(())")]
    ...
    public void NoncompletingUnitDefaultSkipsTheCallee()
        => ScalarEmissionTest.EmitFixture(Prefix + "UnitNever", "func f(x?: () = (loop => continue)) => writeLine(\"bad\")\nwriteLine(\"begin\")\nf()", "begin\n", 0, timeoutMilliseconds: 250);
    ...
    public void NoncompletingDefaultSkipsLaterDefaultsAndTheCallee()
        => ScalarEmissionTest.EmitFixture(Prefix + "TransferNever", "func f(x?: i32 = (loop => continue), y?: i32 = 2147483647 + 1) => writeLine(\"bad\")\nwriteLine(\"begin\")\nf()", "begin\n", 0, timeoutMilliseconds: 250);

    [Fact]
    public void DefaultTransferOperandOverflowAbortsAtItsDeclaration()
    {
        const string Source = "func f(x: i32, y?: i32 = (loop => exit x + 1)) => writeLine(\"bad\")\nf(2147483647)";
    ...
    {
        const string Source = "func f(x: i32, y?: u8 = x@u8) => writeLine(\"bad\")\nf(300)";
    ...
    {
        const string Source = "func f(x: i32, y?: i32 = x + 1) => writeLine(\"bad\")\nf(2147483647)";
        var column = Source.IndexOf("x + 1", StringComparison.Ordinal) + 1;
