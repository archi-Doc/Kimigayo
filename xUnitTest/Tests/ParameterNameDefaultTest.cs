...
public class ParameterNameDefaultTest
{
    ...
    {
        var source = "contract C\n    func f(self, x" + requirement + ": i32) -> i32\n" +
            "struct S\n    Self is C\n    public func f(self, x" + implementation + ": i32) -> i32 => x\n" +
    ...
    public void ReloadAndWarmBindingPreserveIndependentFlags()
    {
        ...
        Assert.True(function.Parameters[0].IsNameOptional);
        Assert.Null(function.Parameters[0].DefaultValue);
        Assert.False(function.Parameters[1].IsNameOptional);
