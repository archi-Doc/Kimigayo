...
public class DynamicArrayCleanupTest
{
    ...
    [InlineData("Receiver", Task + "func make(id: i32) -> Array<Task> => [Task.init(id)]\nrequire not (false and make(1)[0].id == 1) else => $abort(\"skip\")\nrequire true and make(2)[0].id == 2 else => $abort(\"run\")\nConsole.writeLine(\"done\")", "drop 2\ndone\n")]
