...
public class FloatEmissionTest
{
    ...
    public void IeeeComparisonsInfinityAndSignedZero(string type)
    {
        var source = $"""
            ...
            let nan = zero / zero
            if equal(zero, negative) and not unequal(zero, negative) and not less(zero, negative) and lessEqual(zero, negative) and not greater(zero, negative) and greaterEqual(zero, negative) => writeLine("zero")
            if not equal(nan, nan) and unequal(nan, nan) and not less(nan, zero) and not lessEqual(nan, zero) and not greater(nan, zero) and not greaterEqual(nan, zero) => writeLine("nan")
            if not equal(zero, nan) and unequal(zero, nan) and not less(zero, nan) and not lessEqual(zero, nan) and not greater(zero, nan) and not greaterEqual(zero, nan) => writeLine("right")
            if less(negativeInfinity, zero) and greater(positiveInfinity, zero) and equal(positiveInfinity, positiveInfinity) and unequal(positiveInfinity, negativeInfinity) => writeLine("infinity")
            """;
