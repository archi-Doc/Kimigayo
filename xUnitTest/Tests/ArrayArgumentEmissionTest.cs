...
{
    [Theory]
    [InlineData("Integer", "func use(x: [2 of i32]) -> i32 => x[1]\nif use([1, 2]) == 2 => writeLine(\"ok\")")]
    [InlineData("Byte", "func use(x: [1 of u8]) -> u8 => x[0]\nif use([255]) == 255 => writeLine(\"ok\")")]
    [InlineData("Empty", "func use(x: [0 of string]) => writeLine(\"ok\")\nuse([])")]
    [InlineData("Nested", "func use(x: [1 of [1 of string]]) => writeLine(x[0][0])\nuse([[\"ok\"]])")]
    [InlineData("Tuple", "func use(x: [1 of (u8, string)]) => writeLine(x[0].1)\nuse([(255, \"ok\")])")]
    [InlineData("Named", "func use(first: [1 of i32], second: [1 of string]) => writeLine(second[0])\nuse(second: [\"ok\"], first: [1])")]
    [InlineData("ShapeOverload", "func use(x: [0 of i32]) => writeLine(\"wrong\")\nfunc use(x: [1 of i32]) => writeLine(\"ok\")\nuse([1])")]
    [InlineData("RangeOverload", "func use(x: [1 of i8]) => writeLine(\"wrong\")\nfunc use(x: [1 of u8]) => writeLine(\"ok\")\nuse([255])")]
    [InlineData("RangeReversed", "func use(x: [1 of u8]) => writeLine(\"ok\")\nfunc use(x: [1 of i8]) => writeLine(\"wrong\")\nuse([255])")]
    [InlineData("Float", "func use(x: [1 of f32]) -> f32 => x[0]\nif use([1.5]) == 1.5 => writeLine(\"ok\")")]
    [InlineData("Wide", "func use(x: [1 of u128]) -> u128 => x[0]\nif use([340282366920938463463374607431768211455]) > 0 => writeLine(\"ok\")")]
    [InlineData("Unit", "func use(x: [2 of ()]) => writeLine(\"ok\")\nuse([(), ()])")]
    [InlineData("TypedNested", "func use(x: [1 of [1 of u8]]) => writeLine(\"ok\")\nlet row: [1 of u8] = [1]\nuse([row])")]
    ...
    {
        const string Source = "func use(x: [1 of i8]) => writeLine(\"wrong\")\nfunc use(x: [1 of u8]) => writeLine(\"ok\")\nuse([255])";
        var c = MinimalEmissionTest.Analyze(Source);
