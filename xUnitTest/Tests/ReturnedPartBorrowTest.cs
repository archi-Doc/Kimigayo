...
{
    private const string Prefix = "struct Counter\n    public var value: i32 = 1\nstruct Pair\n    public var left: Counter = Counter.init()\n    public var right: Counter = Counter.init()\nfunc relay(p: uniq/Counter) -> uniq/Counter from p => p\nfunc shared(p: ref/Counter) -> ref/Counter from p => p\nfunc second(a: ref/Counter, b: uniq/Counter) -> uniq/Counter from b => b\n";
    ...
    [InlineData("func pick(p: uniq/Pair) -> uniq/Counter from p => p.right@uniq\nvar pair = Pair.init()\nlet a = pick(pair@uniq)\npair.left.value = 2\na.value += 1")]
    [InlineData("func pick(p: uniq/(Pair, Pair)) -> uniq/Pair from p => p.1@uniq\nvar pairs = (Pair.init(), Pair.init())\nlet a = pick(pairs@uniq).left@uniq\npairs.1.left.value = 2\na.value += 1")]
