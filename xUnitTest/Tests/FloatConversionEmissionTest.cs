...
public class FloatConversionEmissionTest
{
    ...
    {
        var source = $"func id(x: {type}) -> {type} => x@{type}\nvar z: {type} = 0.0\nvar one: {type} = 1.0\nlet nan = id(z / z)\nlet zero = id(-0.0)\nlet inf = id(one / z)\nif nan != nan and 1.0 / zero < 0.0 and inf > 1.0 => writeLine(\"ok\")";
