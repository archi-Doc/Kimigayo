...
public class CopySnapshotTest
{
    ...
    // SPEC 13.4.1: a Contract comparison passes shared borrows of both operands, so the left
    // borrow stays live while the right operand exclusively borrows the same Place.
    [Fact]
    public void ContractComparisonKeepsTheLeftBorrowAcrossTheRightOperand()
    {
        const string source = """
            struct Key
                Self is Copy
            ...
            let ordered = mutable@ref < change(mutable@uniq)
            """;
        var c = MinimalEmissionTest.Analyze(source);
        Assert.True(c.Binding.Result.IsComplete, string.Join("\n", c.Binding.Issues));
        Assert.Contains(c.Ownership.Issues, x => x.Failure == OwnershipFailure.ComparisonLoanConflict);
