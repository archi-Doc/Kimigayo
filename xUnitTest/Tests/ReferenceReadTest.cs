...
public class ReferenceReadTest
{
    ...
    [Fact]
    public void AggregateReferentReadsAreRecordedAsUnsupported()
    {
        // Binding accepts a Copy aggregate referent; ownership lowering of that read is a recorded limit.
        var c = MinimalEmissionTest.Analyze("let a: [2 of i32] = [1, 2]\nlet ra = a@ref\nlet b: [2 of i32] = ra\nrequire b[0] == 1 else => $abort(\"aggregate\")");
        Assert.True(c.Binding.Result.IsComplete, string.Join("\n", c.Binding.Issues.Select(x => $"{x.Code}: {x.Node}")));
