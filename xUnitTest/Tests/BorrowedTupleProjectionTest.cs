...
public class BorrowedTupleProjectionTest
{
    ...
    [InlineData("Returned", Counter + "func first(pair: ref/(Counter, bool)) -> ref/Counter from pair => pair.0@ref\nlet pair = (Counter.init(), true)\nlet item = first(pair@ref)\nrequire item.value == 1 else => $abort(\"value\")")]
    ...
    [Theory]
    [InlineData(Counter + "func first(pair: ref/(Counter, bool)) -> ref/Counter from pair => pair.0@ref\nlet item = first((Counter.init(), true))\nlet value = item.value")]
